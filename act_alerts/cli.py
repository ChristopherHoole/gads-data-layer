...
from .email_sender import send_daily_summary, send_rollback_alert, send_performance_alert
...
def cmd_send_test_rollback(args):
    ...
    print(f"Sending test rollback alert for {config.get('client_name')}...")
...
def cmd_send_test_performance(args):
    ...
    print(f"Sending test performance alert for {config.get('client_name')}...")
...
def main():
    ...
    # Send daily summary
    daily_parser = subparsers.add_parser('send-daily-summary', help='Send daily summary email')
    daily_parser.add_argument('config', help='Path to client config YAML')
    daily_parser.add_argument('--date', help='Date for summary (YYYY-MM-DD, default: yesterday)')
    daily_parser.add_argument('--db-path', default='warehouse.duckdb', help='Database path')
    daily_parser.add_argument('--dashboard-url', default='http://localhost:5000', help='Dashboard URL')
    daily_parser.set_defaults(func=cmd_send_daily_summary)
    
    # Send test rollback
    rollback_parser = subparsers.add_parser('send-test-rollback', help='Send test rollback alert')
    rollback_parser.add_argument('config', help='Path to client config YAML')
    rollback_parser.add_argument('--dashboard-url', default='http://localhost:5000', help='Dashboard URL')
    rollback_parser.set_defaults(func=cmd_send_test_rollback)
    
    # Send test performance alert
    perf_parser = subparsers.add_parser('send-test-performance', help='Send test performance alert')
    perf_parser.add_argument('config', help='Path to client config YAML')
    perf_parser.add_argument('--dashboard-url', default='http://localhost:5000', help='Dashboard URL')
