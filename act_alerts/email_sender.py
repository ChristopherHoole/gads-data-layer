...
def send_daily_summary(
    ...
    WHERE customer_id = ?
      AND snapshot_date = ?
    """
    yesterday_data = conn.execute(yesterday_query, [customer_id, snapshot_date]).fetchone()
    ...
    LIMIT 10
    """
    recent_changes = conn.execute(changes_query, [customer_id, snapshot_date]).fetchall()
