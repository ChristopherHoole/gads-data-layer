...
class RollbackExecutor:
    ...
    def execute_rollback_budget(
        ...
        if dry_run:
            print(
                f"  [DRY-RUN] Would set campaign {campaign_id} budget to {target_value_micros / 1_000_000:.2f}"
            )
        ...
        try:
            ...
            print(
                f"  [LIVE] ✅ Set campaign {campaign_id} budget to {target_value_micros / 1_000_000:.2f}"
            )
    ...
    def execute_rollback_bid(
        ...
        if dry_run:
            print(
                f"  [DRY-RUN] Would set campaign {campaign_id} bid target to {target_value_micros / 1_000_000:.2f}"
            )
            return RollbackResult(
                success=True,
                change_id=0,
        ...
        try:
            ...
            print(
                f"  [LIVE] ✅ Set campaign {campaign_id} bid target to {target_value_micros / 1_000_000:.2f}"
            )
    ...
    def execute_rollback(
        self, change: Dict, reason: str, dry_run: bool = True
    ) -> RollbackResult:
        """
        Execute a rollback.
        ...
        old_value = change[
            "old_value"
        ]  # Value BEFORE original change (rollback target)
    ...
    def log_rollback(self, result: RollbackResult, reason: str, original_change: Dict):
        """
        ...
        try:
            ...
            change_pct = (
                (result.new_value - result.old_value) / result.old_value
                if result.old_value > 0
                else 0
            )
            ...
            conn.execute(
                update_query, [rollback_id, reason, datetime.now(), result.change_id]
            )
