...
class ChangeMonitor:
    ...
    def get_baseline_performance(
        ...
        row = conn.execute(
            query, [customer_id, campaign_id, period_start, period_end]
        ).fetchone()
        conn.close()
    ...
    def calculate_performance_delta(
        ...
        def safe_pct_change(old, new):
            if old == 0:
                return (
                    0.0 if new == 0 else 999.0
                )  # Large number if went from 0 to something
            return (new - old) / old
        ...
        conversions_change_pct = safe_pct_change(
            baseline.conversions, current.conversions
        )

        # Conversion value change (higher is better)
        value_change_pct = safe_pct_change(
            baseline.conversion_value_micros, current.conversion_value_micros
        )
...
def format_change_summary(change: MonitoredChange) -> str:
    ...
    lines.append(
        f"Action: {change.lever} {change.old_value:.2f} → {change.new_value:.2f} ({change.change_pct:+.1%})"
    )
    ...
    if change.delta:
        lines.append(f"\nPerformance Change:")
        lines.append(
            f"  CPA: ${change.delta['baseline_cpa']:.2f} → ${change.delta['current_cpa']:.2f} ({change.delta['cpa_change_pct']:+.1%})"
        )
        lines.append(
            f"  ROAS: {change.delta['baseline_roas']:.2f} → {change.delta['current_roas']:.2f} ({change.delta['roas_change_pct']:+.1%})"
        )
        lines.append(
            f"  Conversions: {change.delta['baseline_conversions']:.1f} → {change.delta['current_conversions']:.1f} ({change.delta['conversions_change_pct']:+.1%})"
        )
    else:
