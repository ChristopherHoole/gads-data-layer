...
def cmd_check(args):
    ...
    if changes_confirmed_good:
        ...
        for change, decision in changes_confirmed_good:
            ...
            print(
                f"    Change {change.change_id}: {change.lever} {change.change_pct:+.1%}"
            )
            if change.delta:
                print(
                    f"    CPA: {change.delta['cpa_change_pct']:+.1%}, ROAS: {change.delta['roas_change_pct']:+.1%}, Conv: {change.delta['conversions_change_pct']:+.1%}"
                )
    ...
    if changes_insufficient_data:
        ...
        for change, decision in changes_insufficient_data:
            print(f"  • Campaign {change.campaign_name} ({change.campaign_id})")
            print(
                f"    Change {change.change_id}: {change.lever} {change.change_pct:+.1%} on {change.change_date}"
            )
...
def cmd_rollback(args):
    ...
    for change, change_dict, decision in changes_to_rollback:
        ...
        result = executor.execute_rollback(
            change=change_dict, reason=decision.reason, dry_run=dry_run
        )
    ...
    report_path = (
        report_dir / f"{datetime.now().strftime('%Y-%m-%d_%H%M%S')}_rollbacks.json"
    )
...
Examples:
  ...
  python -m act_radar.cli rollback configs/client_synthetic.yaml --live
    ...
    parser_check = subparsers.add_parser(
        "check", help="Check which changes are being monitored"
    )
    parser_check.add_argument("config", help="Path to client config YAML file")
    parser_check.set_defaults(func=cmd_check)

    # Rollback command
    parser_rollback = subparsers.add_parser(
        "rollback", help="Execute rollbacks for regressed changes"
    )
    parser_rollback.add_argument("config", help="Path to client config YAML file")
    ...
    parser_rollback.add_argument(
        "--live", action="store_true", help="Execute real rollbacks via Google Ads API"
    )
