...
def send_rollback_alert(
    ...
    print(f"  Original Action: {change['old_value']:.2f} → {change['new_value']:.2f} ({change['change_pct']:+.1%})")
    ...
    if 'cpa_before' in evidence:
        print(f"  CPA: ${evidence['cpa_before']:.2f} → ${evidence['cpa_after']:.2f} ({evidence['cpa_change_pct']:+.1%})")
        print(f"  Conversions: {evidence['conversions_before']:.1f} → {evidence['conversions_after']:.1f} ({evidence['conversions_change_pct']:+.1%})")
    
    if 'roas_before' in evidence:
        print(f"  ROAS: {evidence['roas_before']:.2f} → {evidence['roas_after']:.2f} ({evidence['roas_change_pct']:+.1%})")
        print(f"  Conv Value: ${evidence['value_before']:.2f} → ${evidence['value_after']:.2f} ({evidence['value_change_pct']:+.1%})")
...
def send_monitoring_summary(
    ...
    confirmed_good = sum(1 for d in rollback_decisions if not d.should_rollback and d.trigger == 'NONE')
    insufficient_data = sum(1 for d in rollback_decisions if d.trigger == 'INSUFFICIENT_DATA')
...
def format_performance_summary(delta: Dict) -> str:
    ...
    lines.append(f"  CPA: ${delta['baseline_cpa']:.2f} → ${delta['current_cpa']:.2f} ({delta['cpa_change_pct']:+.1%})")
    lines.append(f"  ROAS: {delta['baseline_roas']:.2f} → {delta['current_roas']:.2f} ({delta['roas_change_pct']:+.1%})")
    lines.append(f"  Conversions: {delta['baseline_conversions']:.1f} → {delta['current_conversions']:.1f} ({delta['conversions_change_pct']:+.1%})")
    lines.append(f"  Conv Value: ${delta['baseline_conversion_value']:.2f} → ${delta['current_conversion_value']:.2f} ({delta['value_change_pct']:+.1%})")
    lines.append(f"  Cost: ${delta['baseline_cost']:.2f} → ${delta['current_cost']:.2f} ({delta['cost_change_pct']:+.1%})")
