...
def update_campaign_bidding_strategy(
    ...
    if bid_type not in ['target_roas', 'target_cpa']:
        raise ValueError(f"Invalid bid_type: {bid_type}. Must be 'target_roas' or 'target_cpa'")
    ...
    try:
        ...
        if bid_type == 'target_roas':
            ...
            logger.info(f"Setting target_cpa to {new_target_value} ({target_cpa_micros} micros)")
