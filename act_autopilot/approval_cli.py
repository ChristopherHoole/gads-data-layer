...
class ApprovalCLI:
    ...
    def _review_recommendation(self, idx: int, total: int, rec: Dict[str, Any]) -> str:
        ...
        if (
            rec["current_value"] is not None
            and rec["recommended_value"] is not None
            and rec["change_pct"] is not None
        ):
            print(
                f"  Change: {rec['current_value']:.2f} → {rec['recommended_value']:.2f} ({rec['change_pct']:+.1%})"
            )
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print(
            "Example: python -m act_autopilot.approval_cli reports/suggestions/Synthetic_Test_Client/2026-02-11.json"
        )
        sys.exit(1)
