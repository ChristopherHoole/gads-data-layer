...
def run_rules(
    ...
    for feat in features:
        ...
        # Budget rules
        campaign_recs = []
        campaign_recs.extend(_safe_call(budget_001_increase_high_roas, ctx, 'budget_001_increase_high_roas'))
        campaign_recs.extend(_safe_call(budget_002_decrease_low_roas, ctx, 'budget_002_decrease_low_roas'))
        campaign_recs.extend(_safe_call(budget_003_emergency_cost_spike, ctx, 'budget_003_emergency_cost_spike'))
        campaign_recs.extend(_safe_call(budget_004_recovery_cost_drop, ctx, 'budget_004_recovery_cost_drop'))
        campaign_recs.extend(_safe_call(budget_005_pacing_reduction, ctx, 'budget_005_pacing_reduction'))
        campaign_recs.extend(_safe_call(budget_006_hold_volatile, ctx, 'budget_006_hold_volatile'))
        
        # Bid rules
        campaign_recs.extend(_safe_call(bid_001_tighten_troas, ctx, 'bid_001_tighten_troas'))
        campaign_recs.extend(_safe_call(bid_002_loosen_troas, ctx, 'bid_002_loosen_troas'))
        campaign_recs.extend(_safe_call(bid_003_hold_cvr_drop, ctx, 'bid_003_hold_cvr_drop'))
        campaign_recs.extend(_safe_call(bid_004_hold_low_data, ctx, 'bid_004_hold_low_data'))
        
        # Status rules
        campaign_recs.extend(_safe_call(status_001_flag_underperformer, ctx, 'status_001_flag_underperformer'))
        campaign_recs.extend(_safe_call(status_002_ctr_crisis, ctx, 'status_002_ctr_crisis'))
    ...
    if features:
        ...
        recommendations.extend(_safe_call(acct_001_pacing_alert, account_ctx, 'acct_001_pacing_alert'))
        recommendations.extend(_safe_call(acct_002_portfolio_rebalance, account_ctx, 'acct_002_portfolio_rebalance'))
        recommendations.extend(_safe_call(acct_003_low_data_warning, account_ctx, 'acct_003_low_data_warning'))
...
def _safe_call(rule_func, ctx: RuleContext, rule_name: str) -> List[Recommendation]:
    ...
    try:
        ...
        print(f"[Autopilot] WARN: Rule {rule_name} failed on campaign {ctx.features.get('campaign_id', 'UNKNOWN')}: {str(e)}")
        return []
