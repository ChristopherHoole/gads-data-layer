...
from .models import Recommendation, AutopilotConfig
...
def _infer_lever(action_type: str) -> str:
    ...
    if "budget" in action_type.lower():
        ...
        "bid" in action_type.lower()
        or "troas" in action_type.lower()
        or "tcpa" in action_type.lower()
    ):
        return "bid"
    elif (
        "status" in action_type.lower()
        or "pause" in action_type.lower()
        or "enable" in action_type.lower()
    ):
        return "status"
    else:
        return "unknown"
...
def run_all_guardrails(
    ...
    daily_changes_today = get_daily_change_count(
        customer_id=config.customer_id, check_date=snapshot_date, db_path=db_path
    )
    ...
    if is_blocked:
        logger.warning(
            f"Campaign {rec.entity_id}: BLOCKED by {len(blocked_reasons)} guardrails"
        )
    else:
...
def check_data_sufficiency(rec: Recommendation) -> bool:
    ...
    conversions = evidence.get("conversions_w7", 0) or evidence.get(
        "conversions_w30", 0
    )
...
def get_daily_change_count(
    customer_id: str, check_date: date, db_path: str = "warehouse.duckdb"
) -> int:
    """
    ...
    try:
        ...
        result = con.execute(
            ...
            WHERE customer_id = ?
              ...
              AND date <= ?
        """,
            [customer_id, month_start, snapshot_date],
        ).fetchone()
        ...
        logger.debug(
            f"Monthly projection: {projected_month_spend:.2f} / {monthly_cap:.2f}"
        )
