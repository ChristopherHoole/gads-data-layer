...
def cmd_execute(args):
    ...
    if not Path(report_path).exists():
        ...
        print(
            f"  python -m act_autopilot.suggest_engine {args.config} {args.snapshot_date}"
        )
        return 1
    ...
    if args.rule_ids:
        ...
        logger.info(
            f"Filtered to {len(recommendations)} recommendations matching rule IDs: {rule_ids}"
        )
        print(
...
def main():
    ...
    execute_parser.add_argument(
        "--live", action="store_true", help="Live mode (default: dry-run)"
    )
    execute_parser.add_argument(
        "--report", help="Path to suggestion report JSON (optional)"
    )
    execute_parser.add_argument(
        "--rule-ids", help="Comma-separated rule IDs to execute (optional)"
    )
    execute_parser.add_argument(
    ...
    execute_parser.add_argument(
        ...
        help="Google Ads API config path",
    )
