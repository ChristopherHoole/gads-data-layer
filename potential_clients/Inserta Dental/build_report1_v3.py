"""
Dental by Design - Report 1: Waste Spend Analysis (v3)
v3: Lead with waste spend as hero metric. All slides show both 2025 and 2026.
Always waste spend + CPA for both years.
"""
...
sl.background.fill.solid();sl.background.fill.fore_color.rgb=WHITE
top_bar(sl,Inches(0.06))
stitle(sl,"Geographic Performance","Wider radius targets have significantly higher CPAs \u2014 postcode targeting would unlock bid control")
badge(sl,"2025 vs 2026")

# Radius tables — both years
rad25=geo25[geo25['Location'].str.contains(r'mi\||km\|',na=False)].copy()
rad25g=rad25.groupby('Location').agg({'Cost':'sum','Clicks':'sum','Conversions':'sum'}).reset_index()
rad25g=rad25g[rad25g['Cost']>0].copy()
rad25g['CPA']=rad25g.apply(lambda r:r['Cost']/r['Conversions'] if r['Conversions']>0 else 999,axis=1)
...
# Build combined rows
rad_rows=[]
for _,r in rad25g.sort_values('Cost',ascending=False).head(4).iterrows():
    cpa_s=f"\u00a3{r['CPA']:.0f}" if r['Conversions']>0 else "No Conv"
    rad_rows.append(["2025",str(r['Location'])[:40],f"\u00a3{r['Cost']:,.0f}",f"{r['Conversions']:.0f}",cpa_s])
for _,r in rad26g.sort_values('Cost',ascending=False).head(4).iterrows():
    cpa_s=f"\u00a3{r['CPA']:.0f}" if r['Conversions']>0 else "No Conv"
    rad_rows.append(["2026",str(r['Location'])[:40],f"\u00a3{r['Cost']:,.0f}",f"{r['Conversions']:.0f}",cpa_s])

add_table(sl,Inches(0.60),Inches(1.30),Inches(7.50),Inches(3.10),
    ["Year","Radius Target","Spend","Conv","CPA"],rad_rows,cw=[0.6,3.4,1.0,0.7,0.8])

ibox(sl,Inches(8.40),Inches(1.30),Inches(4.30),Inches(3.10),
    "The Radius Problem",
    "In 2025, tighter radiuses (10mi, 3mi) delivered \u00a323-67 CPA. "
    "In 2026, the active radiuses are wider (30km, 35km) at \u00a359-93 CPA. "
    "The 35km ring at \u00a393 is nearly double the 30km ring at \u00a359. "
    "Wider radiuses reach patients less likely to travel to Hammersmith. "
    "Radius targeting doesn't allow bid adjustments \u2014 you pay the same for Hammersmith and Croydon.",
    RED,RED_TINT)
...
waste_a2=[
    ...
    (f"Sunday premium (\u00a3{sun26:.0f} vs \u00a3{fri26:.0f} Friday)","21% more per lead","No schedule adjustments",YELLOW),
...
# SAVE
# ══════════════════════════════════════════════════════════════════
output=os.path.join(REPORTS,'01_waste_spend_report_v3.pptx')
prs.save(output)
print(f"\nSaved to: {output}")
