"""
Dental by Design - Report 1: Waste Spend Analysis (v5)
v5: Major PMax update — channel breakdown (81.7% Search), Display waste on mobile games,
junk lead quality (NHS/free seekers), 69.3% waste rate (30-day data), new PMax lead quality slide.
"""
...
# ═══ PMax 30-day deep dive (hardcoded verified data) ═══
PMAX_30D = {
    'spend': 30026,
    'conv': 461,
    'cpa': 65,
    ...
    'st_waste_rate': 69.3,
    'st_waste_spend': 11706,
    'display_impressions': 1048422,
    'display_app_placements': 4700,
    'bookings_30d': 72,
    'real_booking_cpa': 418,
    'neg_keywords': 93,
    'top_junk_terms': [
        ('how to apply for dental implants on the nhs', 31, 12),
        ('free dental implants near me', 30, 24),
        ('how to get free dental implants on nhs', 14, 22),
        ('nhs dental implants over 60', 5, 11),
        ('free dental implants clinical trials near me 2026', 4, 11),
    ],
    'top_display_placements': [
        ('Block Blast! (mobile game)', 90537),
        ('Colour by Number (mobile game)', 84084),
        ('Happy Color (mobile game)', 68293),
        ('Jigsawscapes (puzzle game)', 50041),
        ('Snake Clash! (mobile game)', 11415),
    ],
}
PM = PMAX_30D
DISPLAY_WASTE_MONTHLY = PM['channels']['Display Network']['spend']

print("Data loaded and verified (including PMax 30-day deep dive).")
...
# CHART: PMax channel breakdown (pie)
fig,ax=plt.subplots(figsize=(5,4))
ch_labels=[f"Search\n({PM['channels']['Google Search']['pct']}%)",
           f"Search Partners\n({PM['channels']['Search Partners']['pct']}%)",
           f"Display\n({PM['channels']['Display Network']['pct']}%)",
           f"YouTube+Maps\n(0.8%)"]
ch_vals=[PM['channels']['Google Search']['spend'],PM['channels']['Search Partners']['spend'],
         PM['channels']['Display Network']['spend'],PM['channels']['YouTube']['spend']+PM['channels']['Maps']['spend']]
ch_cols=[CB,CY,CR,'#CCCCCC']
ax.pie(ch_vals,labels=ch_labels,colors=ch_cols,autopct=lambda p:f'\u00a3{p*sum(ch_vals)/100:,.0f}',startangle=90,textprops={'fontsize':9})
ax.set_title('PMax Spend by Channel (Last 30 Days)',fontsize=11,color=CN,fontweight='bold')
plt.tight_layout()
plt.savefig(f"{CHARTS}/pmax_channels.png",dpi=200,bbox_inches='tight'); plt.close()

# CHART: PMax Display placements (horizontal bar)
fig,ax=plt.subplots(figsize=(7,3))
apps=[p[0][:30] for p in PM['top_display_placements']]
imps=[p[1] for p in PM['top_display_placements']]
bars=ax.barh(range(len(apps)),imps,color=CR,height=0.5)
ax.set_yticks(range(len(apps))); ax.set_yticklabels(apps,fontsize=9); ax.invert_yaxis()
for bar,val in zip(bars,imps): ax.text(bar.get_width()+500,bar.get_y()+bar.get_height()/2,f'{val:,}',va='center',fontsize=9,color=CN)
ax.set_xlabel('Impressions',fontsize=10)
ax.xaxis.set_major_formatter(mticker.FuncFormatter(lambda x,_:f'{x:,.0f}'))
ax.set_title('Top Display Placements (Last 30 Days)',fontsize=11,color=CN,fontweight='bold',loc='left')
plt.tight_layout()
plt.savefig(f"{CHARTS}/pmax_display_placements.png",dpi=200,bbox_inches='tight'); plt.close()
...
bullets(sl,Inches(0.70),Inches(3.00),Inches(11.50),Inches(2.60),[
    ...
    f"PMax: 81.7% of spend goes to Search, 6.3% to Display (\u00a3{PM['channels']['Display Network']['spend']:,}/month for 2 conversions on mobile games). 69.3% of PMax search terms generate zero conversions (last 30 days).",
    f"PMax lead quality concern: top converting search terms are NHS/free seekers who will never become paying patients. Only {PM['bookings_30d']} bookings from \u00a3{PM['spend']:,} spend \u2014 real booking CPA is \u00a3{PM['real_booking_cpa']}.",
    f"Desktop CPA \u00a3{dev26['Computers']['cpa']:.0f} vs mobile \u00a3{dev26['Mobile Phones']['cpa']:.0f} ({(dev26['Computers']['cpa']-dev26['Mobile Phones']['cpa'])/dev26['Mobile Phones']['cpa']*100:.0f}% higher) with no device bid adjustments. 17 keywords above \u00a3150 CPA (\u00a3{kwd26['high_spend']:,.0f}).",
])
...
# ── S6: PMAX CHANNEL BREAKDOWN (new in v5) ──
sl=prs.slides.add_slide(prs.slide_layouts[6])
sl.background.fill.solid();sl.background.fill.fore_color.rgb=WHITE
top_bar(sl,Inches(0.06))
...
stitle(sl,"PMax Campaign \u2014 Where the Money Goes",f"81.7% of PMax spend is on Google Search. This is a Search campaign with Display waste bolted on.")
badge(sl,"Last 30 Days")

mcard(sl,Inches(0.60),Inches(1.20),f"\u00a3{PM['spend']:,}","PMax Spend (30 Days)",BLUE,Inches(2.40))
mcard(sl,Inches(3.20),Inches(1.20),"81.7%","Goes to Google Search",NAVY,Inches(2.40))
mcard(sl,Inches(5.80),Inches(1.20),f"\u00a3{PM['channels']['Display Network']['spend']:,}","Display: 2 Conversions",RED,Inches(2.40))
mcard(sl,Inches(8.40),Inches(1.20),"69.3%","ST Waste Rate (30 Days)",RED,Inches(2.20))
mcard(sl,Inches(10.80),Inches(1.20),"93","PMax Negative Keywords",YELLOW,Inches(1.90))

img(sl,f"{CHARTS}/pmax_channels.png",Inches(0.60),Inches(2.60),Inches(4.50),Inches(3.50))

# Channel table
add_table(sl,Inches(5.40),Inches(2.60),Inches(7.30),Inches(2.20),
    ["Channel","Spend","% of Total","Conv","CPA"],
    [["Google Search",f"\u00a3{PM['channels']['Google Search']['spend']:,}","81.7%",f"{PM['channels']['Google Search']['conv']}",f"\u00a3{PM['channels']['Google Search']['cpa']}"],
     ["Search Partners",f"\u00a3{PM['channels']['Search Partners']['spend']:,}","11.2%",f"{PM['channels']['Search Partners']['conv']}",f"\u00a3{PM['channels']['Search Partners']['cpa']}"],
     ["Display Network",f"\u00a3{PM['channels']['Display Network']['spend']:,}","6.3%","2","\u00a3947"],
     ["YouTube",f"\u00a3{PM['channels']['YouTube']['spend']}","0.6%","6","\u00a328"],
     ["Maps",f"\u00a3{PM['channels']['Maps']['spend']}","0.2%","1","\u00a369"]],
    cw=[1.5,1.0,0.8,0.6,0.6])

ibox(sl,Inches(5.40),Inches(5.00),Inches(7.30),Inches(1.40),
    "YoY: PMax CPA Doubling",
    f"2025: \u00a326 CPA (PMax ramping up \u2014 cheap early conversions)\n"
    f"2026 Q1: \u00a3{pm26_cpa:.0f} CPA (+100% increase)\n"
    f"Last 30 days: \u00a3{PM['cpa']} CPA (still rising)\n\n"
    "As PMax scales, it reaches into less relevant territory. The 69.3% search term waste rate (up from 53% in 2025) confirms this.",
    RED,RED_TINT)
footer(sl,6)


# ── S7: PMAX DISPLAY WASTE + LEAD QUALITY (new in v5) ──
sl=prs.slides.add_slide(prs.slide_layouts[6])
sl.background.fill.solid();sl.background.fill.fore_color.rgb=WHITE
top_bar(sl,Inches(0.06))
stitle(sl,"PMax \u2014 Display Waste & Lead Quality",f"\u00a3{PM['channels']['Display Network']['spend']:,}/month on mobile games + top converting terms are NHS/free seekers")
badge(sl,"Critical Finding")

# Display placements
img(sl,f"{CHARTS}/pmax_display_placements.png",Inches(0.60),Inches(1.30),Inches(6.00),Inches(2.50))

ibox(sl,Inches(6.90),Inches(1.30),Inches(5.80),Inches(2.50),
    f"Display: \u00a3{PM['channels']['Display Network']['spend']:,}/month for 2 Conversions",
    f"{PM['display_app_placements']:,} mobile app placements, {PM['display_impressions']:,} impressions.\n\n"
    "Dental implant ads shown on Block Blast, Colour by Number, Happy Color, Jigsawscapes, Snake Clash, King James Bible app, and cricbuzz.com.\n\n"
    "These are not people researching dental implants. This is pure waste.",
    RED,RED_TINT)

# Lead quality — the killer finding
txt(sl,Inches(0.60),Inches(4.10),Inches(12),Inches(0.30),"PMax Lead Quality \u2014 Top Converting Search Terms (Last 30 Days)",14,RED,True)
add_table(sl,Inches(0.60),Inches(4.45),Inches(7.50),Inches(2.00),
    ["Search Term","Conv","CPA","Problem"],
    [[t[0][:45],str(t[1]),f"\u00a3{t[2]}","NHS/free seeker \u2014 not a DBD patient"] for t in PM['top_junk_terms']],
    cw=[3.5,0.6,0.6,2.8])

ibox(sl,Inches(8.40),Inches(4.45),Inches(4.30),Inches(2.00),
    f"Real Booking CPA: \u00a3{PM['real_booking_cpa']}",
    f"Only {PM['bookings_30d']} actual bookings from \u00a3{PM['spend']:,} spend.\n\n"
    "The \u201Clow CPA\u201D is an illusion. PMax finds cheap conversions from people looking for free/NHS implants \u2014 "
    "they fill in forms but never book. The Dengro data confirms it: "
    "13.3% lead-to-booking rate.",
    RED,RED_TINT)
footer(sl,7)


# ── S8: KEYWORD INEFFICIENCY ──
sl=prs.slides.add_slide(prs.slide_layouts[6])
sl.background.fill.solid();sl.background.fill.fore_color.rgb=WHITE
top_bar(sl,Inches(0.06))
...
wins=[
    ("1","Block NHS/free seekers from PMax","Add 'nhs', 'free', 'clinical trial' as account-level negatives immediately",RED),
    ("2","Total negative keyword overhaul","Replace 784 broad match with structured 9-list system",RED),
    ("3","Add Display exclusions to PMax","Remove mobile game apps draining \u00a31,893/month for 2 conversions",RED),
    ("4","Implement device bid adjustments","Reduce desktop bids 20-25%, tablet 30-40%",BLUE),
    ("5","Implement day + hour bid adjustments","Sunday -15%, 7-8am -15%, 4-5pm +10%",BLUE),
    ("6","Review 17 high-CPA keywords (\u00a3150+)","Reduce bids or improve landing pages",YELLOW),
    ("7","Disable auto-apply for negative keyword removal","Stop Google removing your protection",GREEN),
    ("8","Tighten geo: radius \u2192 postcodes","Unlock bid control per postcode area",GREEN),
]
...
# SAVE
# ══════════════════════════════════════════════════════════════════
output=os.path.join(REPORTS,'01_waste_spend_report_v5.pptx')
prs.save(output)
print(f"\nSaved to: {output}")
