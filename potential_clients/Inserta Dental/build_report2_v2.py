"""
Dental by Design - Report 2: Account Structure & Issues (v2)
v2: Fix slides 7,8,9,11 — all slides show both years. Ad strength table corrected.
...
add_table(sl,Inches(5.40),Inches(1.55),Inches(5.50),Inches(2.00),
    ...
    [["Excellent","4","2%","Keep \u2014 best performers"],
     ...
     ["Average","105","54%","Rewrite \u2014 opportunity to improve"],
     ["Poor","68","35%","Rewrite urgently \u2014 dragging QS down"],
     ["Pending","19","10%","New ads \u2014 monitor"]],
    cw=[1.0,0.7,0.7,2.5])
...
# ── S9: LANDING PAGE PERFORMANCE (both years) ──
sl=prs.slides.add_slide(prs.slide_layouts[6])
sl.background.fill.solid();sl.background.fill.fore_color.rgb=WHITE
top_bar(sl,Inches(0.06))
stitle(sl,"Landing Page Performance","CPA varies 7x+ across landing pages \u2014 and has worsened year-on-year")
badge(sl,"2025 vs 2026")

# 2025 table
txt(sl,Inches(0.60),Inches(1.20),Inches(6),Inches(0.30),"2025 \u2014 Top Landing Pages by Spend",12,NAVY,True)
lp25_rows=[]
for u,r in lp25.head(6).iterrows():
    url=str(u).replace('https://dentalbydesign.co.uk','').replace('/promo/google/','/')[:35]
    cs=f"\u00a3{r['CPA']:.0f}" if r['Conversions']>0 else "N/A"
    lp25_rows.append([url,f"\u00a3{r['Cost']:,.0f}",f"{r['Conversions']:.0f}",cs,f"{r['CVR']:.1f}%"])
add_table(sl,Inches(0.60),Inches(1.50),Inches(6.00),Inches(2.30),
    ["Landing Page","Spend","Conv","CPA","CVR"],lp25_rows,cw=[2.2,0.9,0.6,0.6,0.6])

# 2026 table
txt(sl,Inches(6.80),Inches(1.20),Inches(6),Inches(0.30),"2026 \u2014 Top Landing Pages by Spend",12,NAVY,True)
lp26_rows=[]
for u,r in lp26.head(6).iterrows():
    url=str(u).replace('https://dentalbydesign.co.uk','').replace('/promo/google/','/')[:35]
    cs=f"\u00a3{r['CPA']:.0f}" if r['Conversions']>0 else "N/A"
    lp26_rows.append([url,f"\u00a3{r['Cost']:,.0f}",f"{r['Conversions']:.0f}",cs,f"{r['CVR']:.1f}%"])
add_table(sl,Inches(6.80),Inches(1.50),Inches(5.90),Inches(2.30),
    ["Landing Page","Spend","Conv","CPA","CVR"],lp26_rows,cw=[2.1,0.9,0.6,0.6,0.6])

# Key findings
ibox(sl,Inches(0.60),Inches(4.10),Inches(3.80),Inches(2.30),
    "Best Performer (2026)",
    "/dental-implants-offer: \u00a334 CPA, 24.7% CVR\n\n"
    "2025: Same page ran at \u00a367 CPA, 6.8% CVR \u2014 "
    "performance improved as the Financing ad group sends cost-conscious traffic here.",
    GREEN,GREEN_TINT)

ibox(sl,Inches(4.70),Inches(4.10),Inches(3.80),Inches(2.30),
    "Worst Performer (2026)",
    "/teeth-implants: \u00a3111 CPA, 8.5% CVR (\u00a331K spent)\n\n"
    "2025: Same page ran at \u00a382 CPA, 9.3% CVR \u2014 "
    "CPA has risen 35% year-on-year despite being the highest-spend page.",
    RED,RED_TINT)

ibox(sl,Inches(8.80),Inches(4.10),Inches(3.90),Inches(2.30),
    "The Good News",
    "QS data confirms landing pages score well (71% Above Average). "
    "The pages themselves aren't the problem \u2014 it's which traffic reaches which page. "
    "Better ad-to-landing-page matching would reduce CPA across the board.",
    BLUE,BLUE_TINT)
footer(sl,9)
...
# Issue table — the centrepiece
add_table(sl,Inches(0.60),Inches(1.20),Inches(12.10),Inches(5.30),
    ["Priority","Issue","Impact","Data (2025 \u2192 2026)","Recommendation"],
    [
     ["CRITICAL","Ghost conversions (KMG legacy)","1,511 phantom conversions misleading the algorithm","Active both years \u2014 confirmed by Giulio as ignore","Remove KMG conversion actions immediately"],
     ["CRITICAL","ROAS bidding on arbitrary \u00a3300 values","Algorithm chasing sparse booking signals, not leads","30% tROAS (PMax), 60% tROAS (Search)","Switch to tCPA or use real treatment revenue"],
     ["CRITICAL","68% of ads rated Poor/Average","Below Average ad relevance on 71% of keywords","Same ads running both years \u2014 never rewritten","Rewrite all ads \u2014 #1 CPA reduction lever"],
     ["CRITICAL","Dengro Offline Lead attribution","May include non-ad leads, inflating Google Ads credit","4,663 leads \u2014 source attribution unverified","Verify with Giulio (pending)"],
     ["IMPORTANT","No device bid adjustments","Desktop \u00a367\u2192\u00a392 CPA vs mobile \u00a342\u2192\u00a356","Gap widened from 60% to 64%","Reduce desktop bids 20-25%"],
     ["IMPORTANT","No ad schedule adjustments","7am CPA: \u00a356\u2192\u00a374 (worst hour both years)","Spread widened from \u00a317 to \u00a332","Reduce 7-8am bids 15-20%"],
     ["IMPORTANT","No day-of-week adjustments","Sunday CPA: \u00a345\u2192\u00a368, CVR: 4.1% (lowest)","Was barely an issue in 2025, now significant","Reduce Sunday bids 15-20%"],
     ["IMPORTANT","47 campaigns (43 paused)","Structural confusion, legacy agencies","KMG, old experiments dating to 2022","Remove all campaigns paused 6+ months"],
     ["IMPORTANT","Auto-apply enabled","Google removing keywords without oversight","24 changes including high-intent keywords","Disable all except ad rotation"],
     ["MONITOR","784 broad match negatives","May block legitimate converting traffic","Account-level lists affecting all campaigns","Full audit \u2014 replace with phrase/exact"],
     ["MONITOR","Landing page CPA variance","7x+ CPA difference across pages","\u00a318\u2013\u00a3142 range (2026)","Optimise traffic routing to best pages"],
     ["MONITOR","Quality Score declining","Weighted avg: 6.9\u21926.4","Ad relevance is the driver","Ad rewrites will address this"],
    ],
    cw=[1.0,2.2,2.5,2.5,2.8])

rbox(sl,Inches(0.60),Inches(6.15),Inches(12.10),Inches(0.40),
    "4 Critical issues + 5 Important issues + 3 Monitor items = 12 total. The Critical issues affect every conversion and every pound spent in the account.")
footer(sl,11)
...
# SAVE
# ══════════════════════════════════════════════════════════════════
output=os.path.join(REPORTS,'02_account_structure_report_v2.pptx')
prs.save(output)
print(f"\nSaved to: {output}")
