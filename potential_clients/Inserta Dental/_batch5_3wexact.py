...
tier5 = three[(three['Conversions']==0) & (three['Cost']>=1) & (three['Cost']<3)].sort_values('Cost', ascending=False)
print(f'Batch 5 (3 word, zero conv, L1-3): {len(tier5)} terms, L{tier5["Cost"].sum():.0f} waste')
print()
for _, r in tier5.iterrows():
    print(f'{r["Search term"]:<50} L{r["Cost"]:>5.2f}')
