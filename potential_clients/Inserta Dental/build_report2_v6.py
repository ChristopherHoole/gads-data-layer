"""
Dental by Design - Report 2: Account Structure & Issues (v6)
v6: Major PMax update — channel breakdown, Display waste on mobile games,
junk lead quality (NHS/free seekers), real booking CPA L418. New PMax deep-dive slide.
"""
...
# PMax 30-day verified data (hardcoded)
PM = {
    ...
    'st_waste_rate': 69.3, 'st_waste_spend': 11706,
    'display_impressions': 1048422, 'display_app_placements': 4700,
...
print("Data loaded (including PMax 30-day deep dive).")
...
# Chart 3: Landing page CPA YoY — HARDCODED VERIFIED DATA
# Verified against raw files on 8 Apr 2026. Do not recalculate.
lp_verified = [
    ('(homepage)', 19, 18),
    ('/dental-implants-offer', 67, 34),
    ('/invisalign', 37, 45),
    ('/cheapest-dental-implants', 50, 83),
    ('/teeth-implants', 82, 111),
    ('/all-on-4-offer', 75, 114),
    ('/dental-implants-hammersmith', 76, 142),
]

fig,ax=plt.subplots(figsize=(10,5))
labels_lp=[p[0] for p in lp_verified]
v25_lp=[p[1] for p in lp_verified]
v26_lp=[p[2] for p in lp_verified]
y=np.arange(len(labels_lp)); w=0.35
b1=ax.barh(y-w/2,v25_lp,w,label='2025',color=CB,alpha=0.8)
b2=ax.barh(y+w/2,v26_lp,w,label='2026 (Q1)',color=CR,alpha=0.8)
ax.set_yticks(y); ax.set_yticklabels(labels_lp,fontsize=10)
ax.invert_yaxis(); ax.legend(fontsize=10)
for bar,val in zip(b1,v25_lp):
    ax.text(bar.get_width()+1,bar.get_y()+bar.get_height()/2,f'\u00a3{val}',va='center',fontsize=9,color=CN,fontweight='bold')
for bar,val in zip(b2,v26_lp):
    ax.text(bar.get_width()+1,bar.get_y()+bar.get_height()/2,f'\u00a3{val}',va='center',fontsize=9,color=CR,fontweight='bold')
ax.set_xlabel('CPA (\u00a3)',fontsize=10)
ax.xaxis.set_major_formatter(mticker.FuncFormatter(lambda x,_:f'\u00a3{x:.0f}'))
ax.set_title('Landing Page CPA: 2025 vs 2026 (All 7 Active Pages)',fontsize=12,color=CN,fontweight='bold',loc='left')
plt.tight_layout()
...
# Chart 6: PMax channel pie (reuse from Report 1 charts folder)
fig,ax=plt.subplots(figsize=(5,4))
ch_labels=[f"Search\n(81.7%)",f"Search Partners\n(11.2%)",f"Display\n(6.3%)",f"YouTube+Maps\n(0.8%)"]
ch_vals=[24537,3358,1893,238]
...
plt.tight_layout()
plt.savefig(f"{CHARTS}/r2_pmax_channels.png",dpi=200,bbox_inches='tight'); plt.close()
...
bullets(sl,Inches(0.70),Inches(3.00),Inches(11.50),Inches(2.60),[
    "1,511 ghost conversions from legacy KMG landing pages inflating reported performance. Giulio confirmed these should be ignored \u2014 but they\u2019re still Primary.",
    f"PMax: 81.7% of spend is Google Search. Display burns \u00a31,893/month on mobile games for 2 conversions. Top converting terms are NHS/free seekers. Real booking CPA is \u00a3{PM['real_booking_cpa']}.",
    f"Non-brand CPA risen {(NBCPA26-NBCPA25)/NBCPA25*100:.0f}% YoY (\u00a3{NBCPA25:.0f}\u2192\u00a3{NBCPA26:.0f}). Monthly spend nearly doubled from ~\u00a322K to ~\u00a358K.",
    f"90% of ads rated Poor/Average. The main Search campaign (\u00a3{c26[c26['Campaign']=='Dental Implants Intent']['Cost'].sum():,.0f} spend) has Poor-rated ads. Ad relevance Below Average on 71% of keywords.",
    "ROAS bidding based on arbitrary \u00a3300 values + PMax only has 93 negative keywords vs 2,192 across the account \u2014 essentially unprotected.",
])
...
# ── S5: PMAX DEEP DIVE (new in v6) ──
sl=prs.slides.add_slide(prs.slide_layouts[6])
sl.background.fill.solid();sl.background.fill.fore_color.rgb=WHITE
top_bar(sl,Inches(0.06))
stitle(sl,"PMax \u2014 Deep Dive","81.7% of PMax spend is Google Search. Display waste on mobile games. Top leads are NHS/free seekers.")
badge(sl,"Last 30 Days")

# Channel pie + table
img(sl,f"{CHARTS}/r2_pmax_channels.png",Inches(0.60),Inches(1.30),Inches(4.50),Inches(3.50))

add_table(sl,Inches(5.40),Inches(1.30),Inches(7.30),Inches(2.20),
...
ibox(sl,Inches(5.40),Inches(3.70),Inches(7.30),Inches(1.10),
    "Display: \u00a31,893/month for 2 Conversions",
    f"{PM['display_app_placements']:,} mobile app placements including Block Blast, Colour by Number, Happy Color, Jigsawscapes, Snake Clash. "
    "These are not dental implant patients. This is pure waste.",
    RED,RED_TINT)

# Lead quality
txt(sl,Inches(0.60),Inches(5.10),Inches(5.50),Inches(0.30),"Top PMax Converting Terms (Last 30 Days)",12,RED,True)
add_table(sl,Inches(0.60),Inches(5.40),Inches(5.50),Inches(1.10),
    ["Search Term","Conv","CPA"],
    [[t[0][:42],str(t[1]),f"\u00a3{t[2]}"] for t in PM['top_junk_terms'][:4]],
    cw=[3.5,0.6,0.6])

ibox(sl,Inches(6.40),Inches(5.10),Inches(6.30),Inches(1.40),
    f"Real Booking CPA: \u00a3{PM['real_booking_cpa']}",
    f"Only {PM['bookings_30d']} actual bookings from \u00a3{PM['spend']:,} spend. "
    "The \u201Clow CPA\u201D is an illusion \u2014 PMax finds cheap conversions from NHS/free seekers who never become patients. "
    f"Only 93 negative keywords protect PMax vs 2,192 across the account.",
    RED,RED_TINT)
footer(sl,5)


# ── S6: ACCOUNT STRUCTURE (was S5) ──
sl=prs.slides.add_slide(prs.slide_layouts[6])
sl.background.fill.solid();sl.background.fill.fore_color.rgb=WHITE
top_bar(sl,Inches(0.06))
...
multipara(sl,Inches(0.80),Inches(1.80),Inches(3.40),Inches(4.40),[
    ...
    ("1,511 phantom conversions misleading algorithm. Confirmed by Giulio.",10,BLACK,False,8),
    ("2. PMax lead quality",11,NAVY,True,3),
    ("Top converting terms are NHS/free seekers. Real booking CPA \u00a3418. Only 93 negatives protecting PMax.",10,BLACK,False,8),
    ("3. PMax Display waste",11,NAVY,True,3),
    ("\u00a31,893/month on mobile games for 2 conversions. 81.7% of PMax spend is Search anyway.",10,BLACK,False,8),
    ("4. ROAS on arbitrary \u00a3300 values",11,NAVY,True,3),
    ("Algorithm chasing sparse bookings, not optimising for lead volume.",10,BLACK,False,8),
    ("5. 90% of ads Poor/Average",11,NAVY,True,3),
    ("Below Average ad relevance on 71% of keywords. Never rewritten.",10,BLACK,False,0),
])
...
rbox(sl,Inches(0.60),Inches(6.15),Inches(12.10),Inches(0.40),
    "The 5 Critical issues affect every conversion and every pound spent. PMax lead quality and Display waste are new findings that change the restructure plan fundamentally.")
footer(sl,12)
...
wins=[
    ("1","Remove KMG ghost conversions + soft signals","Stop 1,511 phantoms + 445 soft signals misleading the algorithm",RED),
    ("2","Block NHS/free seekers from PMax","Account-level negatives for 'nhs', 'free', 'clinical trial' immediately",RED),
    ("3","Add Display exclusions to PMax","Remove mobile game apps burning \u00a31,893/month for 2 conversions",RED),
    ("4","Rewrite all Poor/Average ads","The #1 lever for QS improvement and CPA reduction",BLUE),
    ("5","Evaluate PMax \u2192 Search migration","Build structured Search campaigns to replace PMax over 90 days",BLUE),
    ("6","Evaluate bid strategy: tCPA vs tROAS","Replace arbitrary \u00a3300 values with real revenue or target CPA",GREEN),
]
...
minicard(sl,Inches(6.80),Inches(3.20),"5","Critical Issues",RED)
...
# SAVE
# ══════════════════════════════════════════════════════════════════
output=os.path.join(REPORTS,'02_account_structure_report_v6.pptx')
prs.save(output)
print(f"\nSaved to: {output}")
