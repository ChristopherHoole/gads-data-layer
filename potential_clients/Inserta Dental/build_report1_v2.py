"""
Dental by Design - Report 1: Waste Spend Analysis (v2)
Year-on-year split (2025 vs 2026), corrected numbers, two-angle waste framing.
...
txt(sl,Inches(7.10),Inches(0.60),Inches(5.40),Inches(0.35),"2026 Non-Brand CPA",14,NAVY)
txt(sl,Inches(7.10),Inches(1.00),Inches(5.40),Inches(0.80),f"\u00a3{NBCPA26:.0f}",44,RED,True)
txt(sl,Inches(7.10),Inches(1.80),Inches(5.40),Inches(0.50),f"Up from \u00a3{NBCPA25:.0f} in 2025 \u2014 a {(NBCPA26-NBCPA25)/NBCPA25*100:.0f}% increase while spend nearly doubled",11,BLACK)

minicard(sl,Inches(6.80),Inches(3.20),f"\u00a3{S26:,.0f}","2026 Spend (Q1)",BLUE)
minicard(sl,Inches(8.83),Inches(3.20),f"\u00a3{ZS26:,.0f}","2026 ST Waste",RED)
minicard(sl,Inches(10.87),Inches(3.20),f"{ZP26:.0f}%","Waste Rate",YELLOW)
bot_bar_title(sl)
...
sl.background.fill.solid();sl.background.fill.fore_color.rgb=WHITE
top_bar(sl,Inches(0.06))
stitle(sl,"Waste by Category (2026)","Zero-conversion search terms grouped by intent type")
badge(sl,"Search Term Categories")

img(sl,f"{CHARTS}/waste_cats_2026.png",Inches(0.60),Inches(1.30),Inches(7.00),Inches(4.00))

txt(sl,Inches(8.00),Inches(1.30),Inches(4.70),Inches(0.30),"Category Breakdown",14,NAVY,True)
cat_items=[]
for cat,r in cat26.iterrows():
    cat_items.append(f"{cat}: \u00a3{r['Cost']:,.0f} ({r['Search term']:.0f} terms, {r['Clicks']:.0f} clicks)")
bullets(sl,Inches(8.00),Inches(1.70),Inches(4.70),Inches(3.60),cat_items)

rbox(sl,Inches(0.60),Inches(5.55),Inches(12.13),Inches(0.80),
    f"\"Other Non-Converting\" (\u00a3{cat26.loc['Other Non-Converting','Cost']:,.0f}) includes dental implant terms that are relevant but didn't convert \u2014 "
    "these need landing page and ad copy optimization, not just negative keywords. "
    f"\"Price Research\" (\u00a3{cat26.loc['Price Research (No Location)','Cost']:,.0f}) are people comparing costs without location intent \u2014 blockable with negative keywords.")
footer(sl,5)
...
sl.background.fill.solid();sl.background.fill.fore_color.rgb=WHITE
top_bar(sl,Inches(0.06))
stitle(sl,"Keyword Inefficiency (2026)",f"17 keywords running at CPA above \u00a3150 \u2014 spending \u00a3{kwd26['high_spend']:,.0f} at 2-5x the account average")
...
ibox(sl,Inches(0.60),Inches(4.30),Inches(5.80),Inches(2.00),
    f"{kwd26['zero_count']} Keywords, \u00a3{kwd26['zero_spend']:,.0f} Wasted",
    "These keywords have generated clicks but zero patient enquiries. "
    "They should be paused and their budget redirected to converting keywords.",
    RED,RED_TINT)

ibox(sl,Inches(6.80),Inches(4.30),Inches(5.80),Inches(2.00),
    f"{kwd26['high_count']} Keywords, \u00a3{kwd26['high_spend']:,.0f} at 2-5x CPA",
    "These keywords DO convert \u2014 but at \u00a3150-318 per lead. "
    "They need bid reductions, better ad copy, or dedicated landing pages. "
    "At the current account CPA of \u00a361, anything above \u00a3150 is unsustainable.",
    YELLOW)
footer(sl,7)
...
sl.background.fill.solid();sl.background.fill.fore_color.rgb=WHITE
top_bar(sl,Inches(0.06))
sun_cpa=days26.get('Sunday',{}).get('cpa',0)
fri_cpa=days26.get('Friday',{}).get('cpa',0)
stitle(sl,"Day of Week Performance (2026)",f"Sunday CPA \u00a3{sun_cpa:.0f} \u2014 {(sun_cpa-fri_cpa)/fri_cpa*100:.0f}% above Friday with the lowest conversion rate at {days26.get('Sunday',{}).get('cvr',0):.1f}%")
badge(sl,"Day of Week")

img(sl,f"{CHARTS}/dow_2026.png",Inches(0.60),Inches(1.30),Inches(7.50),Inches(3.50))

ibox(sl,Inches(8.40),Inches(1.30),Inches(4.30),Inches(1.60),
    "Best Days",
    f"Friday: \u00a3{days26.get('Friday',{}).get('cpa',0):.0f} CPA\nSaturday: \u00a3{days26.get('Saturday',{}).get('cpa',0):.0f} CPA\nPatients book toward the end of the week.",
    GREEN,GREEN_TINT)

ibox(sl,Inches(8.40),Inches(3.10),Inches(4.30),Inches(1.60),
    "Worst Day",
    f"Sunday: \u00a3{sun_cpa:.0f} CPA, {days26.get('Sunday',{}).get('cvr',0):.1f}% conversion rate (lowest). "
    "Patients browse on Sunday but book on weekdays.",
    RED,RED_TINT)

rbox(sl,Inches(0.60),Inches(5.10),Inches(12.10),Inches(0.60),
    f"Recommendation: Reduce Sunday bids by 15-20%. Sunday spends \u00a3{days26.get('Sunday',{}).get('spend',0):,.0f} in Q1 2026 at the worst CPA and CVR. No day-of-week bid adjustments currently exist.")
footer(sl,9)
...
sl.background.fill.solid();sl.background.fill.fore_color.rgb=WHITE
top_bar(sl,Inches(0.06))
stitle(sl,"Hour of Day Performance (2026)","7-8am consistently the worst-performing hours at \u00a368-74 CPA")
badge(sl,"Hour of Day")

img(sl,f"{CHARTS}/hod_2026.png",Inches(0.60),Inches(1.30),Inches(12.10),Inches(3.50))

ibox(sl,Inches(0.60),Inches(5.00),Inches(3.80),Inches(1.40),
    "Peak Efficiency",
    "Hours 3-5am and 4-5pm (\u00a342-51 CPA). Late afternoon is both high-volume and efficient.",
    GREEN,GREEN_TINT)

ibox(sl,Inches(4.70),Inches(5.00),Inches(3.80),Inches(1.40),
    "Worst Efficiency",
    "Hour 7am: \u00a374 CPA. Hours 10am-3pm cluster at \u00a368-71 CPA. Early morning and midday peak are expensive.",
    RED,RED_TINT)

ibox(sl,Inches(8.80),Inches(5.00),Inches(3.90),Inches(1.40),
    "Recommendation",
    "Implement ad schedule bid adjustments. Reduce 7-8am by 15-20%, boost 4-5pm by 10%. No scheduling exists.",
    BLUE,BLUE_TINT)
footer(sl,10)
...
sl.background.fill.solid();sl.background.fill.fore_color.rgb=WHITE
top_bar(sl,Inches(0.06))
stitle(sl,"Geographic Performance (2026)","Wider radius targets have significantly higher CPAs \u2014 postcode targeting would unlock bid control")
badge(sl,"Radius & Postcode")

# Radius table (excluding brand)
rad26=geo26[geo26['Location'].str.contains(r'mi\||km\|',na=False)].copy()
rad26g=rad26.groupby('Location').agg({'Cost':'sum','Clicks':'sum','Conversions':'sum'}).reset_index()
rad26g=rad26g[rad26g['Cost']>0].copy()
rad26g['CPA']=rad26g.apply(lambda r:r['Cost']/r['Conversions'] if r['Conversions']>0 else 999,axis=1)
rad_rows=[]
for _,r in rad26g.sort_values('Cost',ascending=False).head(6).iterrows():
    loc=str(r['Location'])[:50]
    cpa_s=f"\u00a3{r['CPA']:.0f}" if r['Conversions']>0 else "No Conv"
    rad_rows.append([loc,f"\u00a3{r['Cost']:,.0f}",f"{r['Conversions']:.0f}",cpa_s])

add_table(sl,Inches(0.60),Inches(1.30),Inches(7.50),Inches(2.50),
    ["Radius Target","Spend","Conv","CPA"],rad_rows,cw=[4.0,1.0,0.8,0.8])

ibox(sl,Inches(8.40),Inches(1.30),Inches(4.30),Inches(2.50),
    "The Radius Problem",
    "The 35km radius spends \u00a37,479 at \u00a393 CPA \u2014 nearly double the 30km ring at \u00a359. "
    "Wider radiuses reach patients who are less likely to travel to Hammersmith. "
    "Radius targeting doesn't allow bid adjustments \u2014 you're paying the same for a patient in Hammersmith as one in Croydon.",
    RED,RED_TINT)

# Postcode data
pc26=geo26[geo26['Location'].str.match(r'^[A-Z]{1,2}\d',na=False)].copy()
pcg26=pc26.groupby('Location').agg({'Cost':'sum','Clicks':'sum','Conversions':'sum'}).reset_index()
pcg26['CPA']=pcg26.apply(lambda r:r['Cost']/r['Conversions'] if r['Conversions']>0 else 999,axis=1)
pc_conv=pcg26[pcg26['Conversions']>0].sort_values('CPA',ascending=False)
pc_rows=[[r['Location'][:20],f"\u00a3{r['Cost']:,.0f}",f"{r['Conversions']:.0f}",f"\u00a3{r['CPA']:.0f}"] for _,r in pc_conv.head(8).iterrows()]

add_table(sl,Inches(0.60),Inches(4.10),Inches(5.50),Inches(2.50),
    ["Postcode","Spend","Conv","CPA"],pc_rows,cw=[1.8,1.0,0.8,0.8])

ibox(sl,Inches(6.40),Inches(4.10),Inches(6.30),Inches(2.50),
    "Recommendation: Postcode Targeting",
    "Replace radius targeting with individual London postcodes. This unlocks bid adjustments per postcode \u2014 "
    "bid higher for W6 (Hammersmith, near the clinic) and lower for outer postcodes where CPA is high. "
    "All campaigns share the same postcode targets, creating one unified view of geographic performance across the account. "
    "This is how you turn geographic data into actionable decisions.",
    GREEN,GREEN_TINT)
footer(sl,11)
...
txt(sl,Inches(7.10),Inches(1.50),Inches(5.30),Inches(0.30),f"\u00a3{kwd26['high_spend']:,.0f}+ in Q1 2026 on keywords with CPA above \u00a3150",11,YELLOW,True)

waste_a2=[
    (f"Desktop premium (\u00a3{dev26['Computers']['cpa']:.0f} vs \u00a3{dev26['Mobile Phones']['cpa']:.0f} mobile)","64% more per lead","No bid adjustments",RED),
    (f"Sunday premium (\u00a3{sun_cpa:.0f} vs \u00a3{fri_cpa:.0f} Friday)","21% more per lead","No schedule adjustments",YELLOW),
    (f"High-CPA keywords (17 at \u00a3150+)",f"\u00a3{kwd26['high_spend']:,.0f} at 2-5x avg","Bid reduction needed",RED),
    (f"35km radius (\u00a393 CPA vs \u00a359 30km)","58% more per lead","Switch to postcodes",YELLOW),
]
...
wins=[
    ("1","Audit 784 broad match negative keywords","Identify which are blocking legitimate converting traffic",RED),
