"""
Dental by Design - Report 2: Account Structure & Issues
Design system: exact match to Objection Experts winning reports.
Both 2025 and 2026 data throughout. Waste spend + CPA for both years.
"""
...
# Chart 3: Landing page CPA comparison 2026
fig,ax=plt.subplots(figsize=(9,4))
lp_data=lp26.head(7)
urls=[str(u).replace('https://dentalbydesign.co.uk','').replace('/promo/google/','/')[:35] for u in lp_data.index]
cpas_lp=lp_data['CPA'].tolist()
colors_lp=[CR if c>100 else CY if c>60 else CG for c in cpas_lp]
bars=ax.barh(range(len(urls)),cpas_lp,color=colors_lp,height=0.5)
ax.set_yticks(range(len(urls))); ax.set_yticklabels(urls,fontsize=9)
ax.invert_yaxis()
for bar,cpa in zip(bars,cpas_lp): ax.text(bar.get_width()+1,bar.get_y()+bar.get_height()/2,f'\u00a3{cpa:.0f}',va='center',fontsize=10,color=CN,fontweight='bold')
ax.set_xlabel('CPA (\u00a3)',fontsize=10)
ax.xaxis.set_major_formatter(mticker.FuncFormatter(lambda x,_:f'\u00a3{x:.0f}'))
ax.set_title('2026 Landing Page CPA',fontsize=11,color=CN,fontweight='bold',loc='left')
plt.tight_layout()
plt.savefig(f"{CHARTS}/lp_cpa_2026.png",dpi=200,bbox_inches='tight'); plt.close()
...
mcard(sl,Inches(6.80),Inches(1.20),"68%","Ads Rated Poor or Average",RED)
...
bullets(sl,Inches(0.70),Inches(3.00),Inches(11.50),Inches(2.60),[
    ...
    f"68% of ads are rated Poor or Average strength. The main Search campaign ({c26[c26['Campaign']=='Dental Implants Intent']['Cost'].sum():,.0f} spend) has Poor-rated ads.",
...
# QS components
txt(sl,Inches(7.40),Inches(1.20),Inches(5.30),Inches(0.30),"QS Components (2026)",14,NAVY,True)
add_table(sl,Inches(7.40),Inches(1.55),Inches(5.30),Inches(1.80),
    ["Component","Below Avg","Average","Above Avg"],
    [["Ad Relevance","58 (71%)","10 (12%)","14 (17%)"],
     ["Expected CTR","52 (63%)","22 (27%)","8 (10%)"],
     ["Landing Page Exp.","3 (4%)","21 (26%)","58 (71%)"]],
    cw=[1.5,1.1,1.1,1.1])

ibox(sl,Inches(7.40),Inches(3.50),Inches(5.30),Inches(1.30),
    "The Pattern",
    "Landing pages are strong (71% Above Average). "
    "Ad relevance is the weak point (71% Below Average). "
    "Expected CTR is also weak (63% Below Average). "
    "The ads don't match what people are searching for.",
    RED,RED_TINT)

# Low QS keywords with spend
low_qs=scored26[scored26['QS']<=3].sort_values('Cost',ascending=False).head(5)
txt(sl,Inches(0.60),Inches(4.50),Inches(6),Inches(0.30),"Worst QS Keywords (QS 1-3) by Spend",12,NAVY,True)
add_table(sl,Inches(0.60),Inches(4.80),Inches(7.00),Inches(1.70),
    ["Keyword","QS","Spend","CPA","Ad Rel.","Exp CTR"],
    [[r['Keyword'][:35],f"{int(r['QS'])}",f"\u00a3{r['Cost']:,.0f}",
      f"\u00a3{r['Cost']/r['Conversions']:.0f}" if r['Conversions']>0 else "N/A",
      str(r['Ad relevance'])[:10],str(r['Exp. CTR'])[:10]]
     for _,r in low_qs.iterrows()],
    cw=[2.5,0.4,0.9,0.7,1.0,1.0])

ibox(sl,Inches(7.90),Inches(4.80),Inches(4.80),Inches(1.70),
    "QS Tax Estimate",
    "Keywords at QS 3 pay roughly 2-3x more per click than QS 7+ keywords. "
    f"With \u00a3{scored26[scored26['QS']<=3]['Cost'].sum():,.0f} spent on QS 1-3 keywords in 2026, "
    "an estimated 30-50% of that spend is a Quality Score premium. "
    "Improving ad relevance is the single biggest lever.",
    YELLOW)
footer(sl,7)
...
sl.background.fill.solid();sl.background.fill.fore_color.rgb=WHITE
top_bar(sl,Inches(0.06))
stitle(sl,"Ad Copy Audit","68% of ads rated Poor or Average \u2014 the primary driver of low Quality Scores")
badge(sl,"2025 vs 2026")

img(sl,f"{CHARTS}/ad_strength_2026.png",Inches(0.60),Inches(1.30),Inches(4.50),Inches(3.00))

# Strength comparison
txt(sl,Inches(5.40),Inches(1.20),Inches(7.30),Inches(0.30),"Ad Strength Year-on-Year",14,NAVY,True)
add_table(sl,Inches(5.40),Inches(1.55),Inches(5.00),Inches(2.00),
    ["Strength","2025","2026"],
    [["Excellent",str(str25.get('Excellent',0)),str(str26.get('Excellent',0))],
     ["Good",str(str25.get('Good',0)),str(str26.get('Good',0))],
     ["Average",str(str25.get('Average',0)),str(str26.get('Average',0))],
     ["Poor",str(str25.get('Poor',0)),str(str26.get('Poor',0))],
     ["Pending",str(str25.get('Pending',0)),str(str26.get('Pending',0))]],
    cw=[1.0,0.8,0.8])

ibox(sl,Inches(5.40),Inches(3.80),Inches(7.30),Inches(1.00),
    "Critical Finding",
    "The main Search Intent campaign (\u00a374K spend in 2026) has Poor-rated ads. "
    "This directly explains the Below Average ad relevance and expected CTR on most keywords. "
    "Poor ads = low QS = higher CPCs = higher CPA.",
    RED,RED_TINT)

rbox(sl,Inches(0.60),Inches(5.00),Inches(12.10),Inches(0.60),
    "Recommendation: Rewrite all Poor and Average ads. Focus on ad relevance \u2014 match headlines to search intent. Test 2-3 RSA variants per ad group with different messaging angles (price, speed, quality).")

# Key ad groups and their strength
txt(sl,Inches(0.60),Inches(5.80),Inches(12),Inches(0.30),
    "This is the #1 lever for reducing CPA. Better ads = higher QS = lower CPCs = more patients for the same budget.",
    11,NAVY,True)
footer(sl,8)


# ── S9: LANDING PAGE PERFORMANCE ──
sl=prs.slides.add_slide(prs.slide_layouts[6])
sl.background.fill.solid();sl.background.fill.fore_color.rgb=WHITE
top_bar(sl,Inches(0.06))
stitle(sl,"Landing Page Performance (2026)","CPA ranges from \u00a318 to \u00a3142 across landing pages \u2014 a 7.9x difference")
badge(sl,"Landing Pages")

img(sl,f"{CHARTS}/lp_cpa_2026.png",Inches(0.60),Inches(1.30),Inches(8.00),Inches(3.50))

# Key insight
ibox(sl,Inches(8.90),Inches(1.30),Inches(3.80),Inches(1.60),
    "Best Performer",
    "/dental-implants-offer\n\u00a334 CPA, 24.7% CVR\n\n"
    "The 'Financing' ad group sends traffic here \u2014 matching cost-conscious patients with the right page.",
    GREEN,GREEN_TINT)

ibox(sl,Inches(8.90),Inches(3.10),Inches(3.80),Inches(1.60),
    "Worst Performer",
    "/dental-implants-hammersmith\n\u00a3142 CPA, 3.6% CVR\n\n"
    "This page has the lowest conversion rate in the account. L1,631 spent for only 12 conversions.",
    RED,RED_TINT)

rbox(sl,Inches(0.60),Inches(5.10),Inches(12.10),Inches(0.60),
    "QS confirms landing pages are strong (71% Above Average). The issue is ad-to-landing-page relevance \u2014 the right traffic needs to reach the right page. "
    "The /teeth-implants page at L111 CPA receives the most spend but has one of the highest CPAs.")

ibox(sl,Inches(0.60),Inches(5.90),Inches(12.10),Inches(0.60),
    "Note: 2025 Comparison",
    "In 2025, the /dental-implants-offer page was the top performer at \u00a363 CPA \u2014 "
    "performance has shifted as traffic patterns changed. The landing pages themselves score well on QS.",
    BLUE,BLUE_TINT)
footer(sl,9)
...
# Critical
txt(sl,Inches(0.60),Inches(1.20),Inches(12),Inches(0.30),"Critical (Fix Immediately)",14,RED,True)
issues_crit=[
    "1,511 ghost conversions (KMG legacy) still set as Primary \u2014 inflating reported CPA and misleading the bidding algorithm",
    "ROAS bidding based on arbitrary L300 values \u2014 algorithm chasing sparse booking signals instead of optimising for lead volume",
    "68% of ads rated Poor/Average \u2014 directly causing Below Average ad relevance on 71% of keywords",
    "Dengro Offline Leads may include non-ad sources \u2014 pending verification, but could further inflate Google Ads attribution",
]
bullets(sl,Inches(0.70),Inches(1.55),Inches(11.90),Inches(1.60),issues_crit,11)

# Important
txt(sl,Inches(0.60),Inches(3.30),Inches(12),Inches(0.30),"Important (Fix Within 30 Days)",14,YELLOW,True)
issues_imp=[
    f"No device bid adjustments \u2014 desktop CPA L{92} vs mobile L{56} (64% higher)",
    "No ad schedule bid adjustments \u2014 7am CPA L74 (worst) vs 3am L42 (best)",
    "No day-of-week bid adjustments \u2014 Sunday CPA L68 vs Friday L56",
    "47 campaigns creating structural confusion \u2014 43 should be removed",
    "24 auto-apply changes removing keywords without human review",
]
bullets(sl,Inches(0.70),Inches(3.65),Inches(11.90),Inches(1.50),issues_imp,11)

# Minor
txt(sl,Inches(0.60),Inches(5.30),Inches(12),Inches(0.30),"Monitor (Ongoing)",14,BLUE,True)
issues_minor=[
    "784 broad match negative keywords at account level \u2014 need full audit",
    "Landing page CPA variance (L18 to L142) \u2014 traffic routing needs optimization",
    "Quality Score declining year-on-year (6.9 to 6.4 weighted average) \u2014 ad rewrites will address this",
]
bullets(sl,Inches(0.70),Inches(5.65),Inches(11.90),Inches(0.90),issues_minor,11)
...
# SAVE
# ══════════════════════════════════════════════════════════════════
output=os.path.join(REPORTS,'02_account_structure_report.pptx')
prs.save(output)
print(f"\nSaved to: {output}")
