"""
Dental by Design - Report 3: Restructure & Growth Plan (v2)
v2: PMax → Search migration strategy. New 90-day plan based on PMax deep-dive findings.
Display waste, NHS/free junk leads, real booking CPA £418.
"""
...
bullets(sl,Inches(0.70),Inches(3.00),Inches(11.50),Inches(2.60),[
    "Week 1: Fix conversion tracking (remove 1,511 KMG ghosts + soft signals), block NHS/free seekers from PMax, add Display exclusions, negative keyword 9-list overhaul.",
    ...
    "Month 2: Scale Search winners based on data. Continue reducing PMax budget. Geo restructure (radius \u2192 postcodes). Device + schedule bid adjustments.",
    "Month 3: Search becomes dominant. PMax reduced to supplementary role (brand awareness only). Multi-clinic rollout planning.",
])
...
sl.background.fill.solid();sl.background.fill.fore_color.rgb=WHITE
top_bar(sl,Inches(0.06))
stitle(sl,"Step 4: Bid Adjustments","Device, schedule, day-of-week \u2014 no adjustments currently exist")
badge(sl,"Weeks 2-4")
...
img(sl,f"{CHARTS}/r3v2_timeline.png",Inches(0.60),Inches(1.10),Inches(12.10),Inches(2.20))
...
for i,(label,title,color,items) in enumerate(phases):
    ...
    bullets(sl,cx+Inches(0.15),Inches(4.25),Inches(2.60),Inches(2.10),items,9)
footer(sl,11)
...
steps=[
    ...
    ("2","Conversion tracking + negative keyword overhaul in Week 1","The two highest-impact changes \u2014 immediate CPA improvement",BLUE),
    ("3","New Search campaigns built and launched by end of Month 1","Treatment-specific structure with proper ads, negatives, tCPA",YELLOW),
...
# SAVE
# ══════════════════════════════════════════════════════════════════
output=os.path.join(REPORTS,'03_restructure_report_v2.pptx')
prs.save(output)
print(f"\nSaved to: {output}")
