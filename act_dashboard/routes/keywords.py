...
def keywords():
    ...
    return render_template(
        ...
        # Chat 108: keyword list for flow builder
        rfb_keywords=[
            {'id': str(kw.get('keyword_id', '') or ''), 'name': str(kw.get('keyword_text', '') or '')}
...
# ══════════════════════════════════════════════════════════════════════════════
# Chat 108: Keyword Rules & Flags CRUD routes
# Pattern: Copied from ad_groups.py (Chat 107), adapted for entity_type='keyword'
# ══════════════════════════════════════════════════════════════════════════════

_KW_WAREHOUSE_PATH = _Path(__file__).parent.parent.parent / 'warehouse.duckdb'


def _kw_get_warehouse():
    """Open warehouse.duckdb writable."""
    return duckdb.connect(str(_KW_WAREHOUSE_PATH))


def _kw_client_cfg_name():
...
def _kw_serialize_rule(row, cols):
...
@bp.route("/keywords/rules", methods=['GET'])
@login_required
def kw_list_rules():
    """GET /keywords/rules — return all keyword rules + flags for client."""
    client_config = _kw_client_cfg_name()
    if not client_config:
        return jsonify({'success': False, 'error': 'No client selected'}), 400

    conn = _kw_get_warehouse()
    try:
        result = conn.execute(
            "SELECT * FROM rules WHERE client_config = ? AND entity_type = 'keyword' ORDER BY type, id",
            [client_config]
        )
        cols = [d[0] for d in result.description]
        rows = [_kw_serialize_rule(row, cols) for row in result.fetchall()]
...
@bp.route("/keywords/rules", methods=['POST'])
@login_required
def kw_create_rule():
    """POST /keywords/rules — insert new keyword rule/flag."""
    client_config = _kw_client_cfg_name()
    if not client_config:
        return jsonify({'success': False, 'error': 'No client selected'}), 400
    ...
    if not is_template:
        try:
            ...
            print(f"[Chat 108] Duplicate check error: {e}")
    ...
    try:
        ...
        cols = [d[0] for d in result.description]
        new_row = _kw_serialize_rule(result.fetchone(), cols)
        print(f"[Chat 108] Created keyword rule id={next_id} name='{data['name']}'")
...
@bp.route("/keywords/rules/<int:rule_id>", methods=['PUT'])
@login_required
def kw_update_rule(rule_id):
    """PUT /keywords/rules/<rule_id> — full replace of mutable columns."""
    client_config = _kw_client_cfg_name()
    if not client_config:
        return jsonify({'success': False, 'error': 'No client selected'}), 400
    ...
    try:
        ...
        conn.execute("""
            UPDATE rules SET
                name = ?, rule_or_flag = ?, type = ?,
                campaign_type_lock = ?,
                entity_scope = ?, conditions = ?,
                action_type = ?, action_magnitude = ?,
                cooldown_days = ?, risk_level = ?,
                is_template = ?, updated_at = NOW()
        ...
        """, [
            ...
            data.get('cooldown_days', 7), data.get('risk_level'),
            is_template, rule_id, client_config,
        ])
        conn.commit()

        result = conn.execute("SELECT * FROM rules WHERE id = ?", [rule_id])
        cols = [d[0] for d in result.description]
        updated = _kw_serialize_rule(result.fetchone(), cols)
        print(f"[Chat 108] Updated keyword rule id={rule_id}")
...
@bp.route("/keywords/rules/<int:rule_id>", methods=['DELETE'])
@login_required
def kw_delete_rule(rule_id):
    """DELETE /keywords/rules/<rule_id>."""
    client_config = _kw_client_cfg_name()
    if not client_config:
        return jsonify({'success': False, 'error': 'No client selected'}), 400
    conn = _kw_get_warehouse()
    try:
        ...
        conn.commit()
        print(f"[Chat 108] Deleted keyword rule id={rule_id}")
...
@bp.route("/keywords/rules/<int:rule_id>/toggle", methods=['POST'])
@login_required
def kw_toggle_rule(rule_id):
    """POST /keywords/rules/<rule_id>/toggle — flip enabled."""
    client_config = _kw_client_cfg_name()
    if not client_config:
        return jsonify({'success': False, 'error': 'No client selected'}), 400
    conn = _kw_get_warehouse()
...
@bp.route("/keywords/rules/<int:rule_id>/save-as-template", methods=['POST'])
@login_required
def kw_save_as_template(rule_id):
    """POST /keywords/rules/<rule_id>/save-as-template."""
    client_config = _kw_client_cfg_name()
    if not client_config:
        return jsonify({'success': False, 'error': 'No client selected'}), 400
    conn = _kw_get_warehouse()
    try:
        ...
        conn.execute("""
            INSERT INTO rules (
                ...
                action_type, action_magnitude,
                cooldown_days, risk_level, enabled,
                is_template, plain_english, created_at, updated_at
            ) VALUES (
                ?, ?, 'keyword', ?,
                ?, ?, ?,
                ?, ?, ?, ?,
        ...
        """, [
            ...
            orig_dict.get('cooldown_days', 7), orig_dict.get('risk_level'),
        ...
        cols = [d[0] for d in result.description]
        new_row = _kw_serialize_rule(result.fetchone(), cols)
        print(f"[Chat 108] Saved keyword rule id={rule_id} as template id={next_id}")
        return jsonify({'success': True, 'data': new_row})
    except Exception as e:
        import traceback; traceback.print_exc()
        return jsonify({'success': False, 'error': str(e)}), 500
