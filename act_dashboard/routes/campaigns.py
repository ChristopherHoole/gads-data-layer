...
def load_campaign_data(
    ...
    if date_from and date_to:
        # Custom date range — dates validated before storage in session
...
def campaigns():
    ...
    Date range is now read from Flask session (set via POST /set-date-range).
    Falls back to ?days= URL param for backward compatibility only.
    """
