...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ═════════════════════════════════════════════════════════════════════════
  // v2: Optimization Levels — 3-state toggle handling
  // ═════════════════════════════════════════════════════════════════════════
  ...
  // Initialise all rows to "off" (default state for a new client)
  document.querySelectorAll('.level-row').forEach(row => applyLevelState(row, 'off'));
  ...
  document.querySelectorAll('.level-toggle').forEach(toggle => {
    toggle.querySelectorAll('.level-toggle__opt').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        updateLevelsTimestamp();
        showToast(`${levelName}: ${STATE_LABELS[newState]}`, newState === 'active' ? 'success' : newState === 'monitor' ? 'info' : 'info');
      });
    });
  });

  function updateLevelsTimestamp() {
    const now = new Date();
    const opts = { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit', hour12: true };
    const formatted = now.toLocaleString('en-GB', opts).replace(',', ' ·');
    const el = document.getElementById('levelsLastUpdated');
