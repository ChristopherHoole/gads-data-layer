...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function mkRows(scale) {
    const base = [
      ...
      { kw: 'dental implant clinic london', match: 'phrase', camp: 'GLO Campaign — Core',       ag: 'Long Tail',        maxCpc: 7.50, qs: 0, cost30: 144, impr30: 1620, clicks30: 84, conv30: 8, flags: 1, flagType: 'discovery' },
    ];
    return base.map(r => {
  ...
  function matchBadgeHtml(m) {
    ...
    return `<span class="match-badge match-badge--${m}">${label}</span>`;
  }
  ...
  function updateTable(range) {
    ...
    rows.forEach(r => {
      ...
      tr.innerHTML = `
        ...
        <td>${matchBadgeHtml(r.match)}</td>
        <td><span class="parent-campaign-badge">${r.camp}</span></td>
        <td><span class="parent-adgroup-badge">${r.ag}</span></td>
    ...
    if (info) info.textContent = `Showing 1-${rows.length} of 142 keywords`;
  }
  ...
  const KW_DATA = {
    'dental consultation free': {
      ...
      cost: '£125', cpa: '—', cpaZone: 'underperforming', cpaDetail: 'No conversions in 72 days — Underperforming',
      conv: '0', qs: 2,
      approvals: [{
        text: '<strong>Pause &ldquo;dental consultation free&rdquo;</strong> &mdash; 0 conversions in 72 days despite £125 spend.',
    ...
    'dental implant clinic london': {
      ...
      cost: '£144 (via search term)', cpa: '£18.00', cpaZone: 'outperforming', cpaDetail: '28% below target — Outperforming',
      conv: '8', qs: '—',
      approvals: [{
  ...
  function openKwSlidein(kw) {
    ...
    if (kwMeta) kwMeta.innerHTML = `
      <span class="match-badge match-badge--${d.match}">${d.match.charAt(0).toUpperCase() + d.match.slice(1)}</span>
      <span class="parent-campaign-badge">${d.camp}</span>
      <span class="parent-adgroup-badge">${d.ag}</span>
      <span style="font-size:14px;color:var(--act-text)"><span class="status-dot status-dot--${d.status === 'pending' ? 'paused' : d.status}"></span> ${d.status === 'pending' ? 'Pending approval' : 'Enabled'}</span>`;

    let html = '';

    // Health cards (4 in a row)
    html += '<div class="kw-health-grid">';
    html += `<div class="kw-health-card"><div class="kw-health-card__label">Cost (MTD)</div><div class="kw-health-card__value">${d.cost}</div></div>`;
    const cpaColor = d.cpaZone === 'outperforming' ? '#10b981' : d.cpaZone === 'underperforming' ? '#ef4444' : '#3b82f6';
    html += `<div class="kw-health-card"><div class="kw-health-card__label">CPA</div><div class="kw-health-card__value" style="color:${cpaColor}">${d.cpa}</div><div class="kw-health-card__detail"><span class="zone-badge zone-badge--${d.cpaZone}">${d.cpaDetail}</span></div></div>`;
    html += `<div class="kw-health-card"><div class="kw-health-card__label">Conversions (MTD)</div><div class="kw-health-card__value">${d.conv}</div></div>`;
    const qsColor = typeof d.qs === 'number' ? (d.qs < 4 ? '#ef4444' : d.qs <= 6 ? '#f59e0b' : '#10b981') : 'var(--act-text)';
    html += `<div class="kw-health-card"><div class="kw-health-card__label">Quality Score</div><div class="kw-health-card__value" style="color:${qsColor}">${d.qs}${typeof d.qs === 'number' ? '<span style="font-size:14px;opacity:0.5"> / 10</span>' : ''}</div></div>`;
    html += '</div>';
