...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function flagBadgeHtml(type) {
    if (!type) return '—';
    const labels = { negative: 'Negative Outlier', positive: 'Positive Outlier', pause: 'Pause Candidate', concentration: 'Concentration' };
    return `<span class="flag-count-badge"><span class="material-symbols-outlined">flag</span>${labels[type] || type}</span>`;
  }
  ...
  function updateTable(range) {
    ...
    rows.forEach(r => {
      ...
      tr.innerHTML = `
        ...
        <td>${r.costconv}</td>
        <td>${r.cvrStr}</td>
        <td>${r.flags > 0 ? flagBadgeHtml(r.flagType) : '—'}</td>`;
  ...
  const AG_DATA = {
    ...
    'Test Batch 3': {
      ...
      approvals: [{
        ...
        rec: 'ACT recommends: Pause this ad group and reallocate budget to better-performing ad groups.',
  ...
  function openAgSlidein(name) {
    ...
    if (d.approvals.length > 0) {
      html += `<div class="slidein-section"><div class="slidein-section__header"><span class="material-symbols-outlined" style="font-size:20px;color:var(--act-amber)">pending_actions</span><span style="font-size:16px;font-weight:600;color:var(--act-text)">Awaiting Approval</span><span style="font-size:12px;font-weight:600;padding:2px 8px;border-radius:10px;color:white;background:var(--act-amber)">${d.approvals.length}</span><span class="material-symbols-outlined slidein-section__toggle">expand_more</span></div><div class="slidein-section__body" style="padding:16px">`;
      d.approvals.forEach((a, idx) => {
        ...
        html += `<div class="act-item" data-priority="investigate">
          <div class="act-item__row">
            <div class="act-item__content">
              <div class="act-item__top">
                <span class="badge-level badge-level--adgroup">Ad Group</span>
    ...
    if (d.executed.length > 0) {
      html += `<div class="slidein-section collapsed"><div class="slidein-section__header"><span class="material-symbols-outlined" style="font-size:20px;color:var(--act-green)">check_circle</span><span style="font-size:16px;font-weight:600;color:var(--act-text)">Actions Executed Overnight</span><span style="font-size:12px;font-weight:600;padding:2px 8px;border-radius:10px;color:white;background:var(--act-green)">${d.executed.length}</span><span class="material-symbols-outlined slidein-section__toggle">expand_more</span></div><div class="slidein-section__body" style="padding:16px">`;
      d.executed.forEach(e => {
        html += `<div class="act-item"><div class="act-item__row"><div class="act-item__content"><div class="act-item__top"><span class="badge-level badge-level--adgroup">Ad Group</span><span class="badge-action badge-action--act">Act</span><span class="act-item__timestamp">${e.time}</span></div><div class="act-item__summary">${e.text}${e.reason ? `<span class="act-item__reason-inline">${e.reason}</span>` : ''}</div></div></div></div>`;
      });
      html += '</div></div>';
    }
    ...
    if (d.monitoring && d.monitoring.length > 0) {
      ...
      d.monitoring.forEach(m => {
        ...
        html += `<div class="act-item"><div class="act-item__row"><div class="act-item__content"><div class="act-item__top"><span class="badge-level badge-level--adgroup">Ad Group</span><span class="badge-action badge-action--monitor">Monitor</span><span class="act-item__started"><span class="material-symbols-outlined">calendar_today</span>Started: ${m.started}</span></div><div class="act-item__summary">${m.text}</div><div class="act-item__progress"><div class="act-item__progress-bar"><div class="act-item__progress-fill" style="width:${m.progress}%;background:${barColor}"></div></div><span class="act-item__progress-text">${m.remaining}</span><span class="health-label ${labelClass}">${m.healthLabel}</span></div></div></div></div>`;
      });
      html += '</div></div>';
    }

    // RECENT ALERTS (collapsed) — show even if empty
    html += `<div class="slidein-section collapsed"><div class="slidein-section__header"><span class="material-symbols-outlined" style="font-size:20px;color:var(--act-red)">notifications_active</span><span style="font-size:16px;font-weight:600;color:var(--act-text)">Recent Alerts</span><span style="font-size:12px;font-weight:600;padding:2px 8px;border-radius:10px;color:white;background:var(--act-red)">${d.alerts.length}</span><span class="material-symbols-outlined slidein-section__toggle">expand_more</span></div><div class="slidein-section__body" style="padding:16px">`;
    if (d.alerts.length === 0) {
      html += '<div style="font-size:14px;color:var(--act-text);opacity:0.6;text-align:center;padding:16px">No alerts in the last 7 days.</div>';
    } else {
      d.alerts.forEach(a => {
        html += `<div class="act-item" data-priority="alert"><div class="act-item__row"><div class="act-item__content"><div class="act-item__top"><span class="badge-level badge-level--adgroup">Ad Group</span><span class="badge-action badge-action--alert">Alert</span><span class="act-item__timestamp">${a.date}</span></div><div class="act-item__summary">${a.text}</div></div></div></div>`;
      });
    }
    html += '</div></div>';

    // OUTLIER ANALYSIS (collapsed) — unique to Ad Group Level
    html += `<div class="slidein-section collapsed"><div class="slidein-section__header"><span class="material-symbols-outlined" style="font-size:20px;color:#f59e0b">analytics</span><span style="font-size:16px;font-weight:600;color:var(--act-text)">Outlier Analysis</span><span class="material-symbols-outlined slidein-section__toggle">expand_more</span></div><div class="slidein-section__body" style="padding:16px">`;
    html += '<table class="outlier-table"><thead><tr><th>Metric</th><th>This Ad Group</th><th>Campaign Average</th><th>Delta</th></tr></thead><tbody>';
    d.outlier.rows.forEach(r => {
      html += `<tr><td>${r.metric}</td><td>${r.self}</td><td>${r.avg}</td><td class="outlier-delta--${r.class}">${r.delta}</td></tr>`;
    });
    html += '</tbody></table>';
    html += `<div class="outlier-note"><span class="material-symbols-outlined">info</span>${d.outlier.note}</div>`;
    html += '</div></div>';

    // STRUCTURAL HEALTH (collapsed) — unique to Ad Group Level
    html += `<div class="slidein-section collapsed"><div class="slidein-section__header"><span class="material-symbols-outlined" style="font-size:20px;color:#f59e0b">health_and_safety</span><span style="font-size:16px;font-weight:600;color:var(--act-text)">Structural Health</span><span class="material-symbols-outlined slidein-section__toggle">expand_more</span></div><div class="slidein-section__body"><ul class="structural-health">`;
    d.structural.forEach(s => {
      html += `<li><span class="material-symbols-outlined">${s.icon}</span>${s.label}<span class="structural-health__value">${s.value}</span></li>`;
    });
    html += '</ul></div></div>';
    ...
    // Bind See Details inline expand
