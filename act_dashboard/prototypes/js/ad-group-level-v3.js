...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // v3: Main page "See Details" expands decision tree INLINE (not slide-in)
  // — matches Morning Review / Campaign Level main page pattern.
  document.querySelectorAll('#sectionApproval [data-action="details"]').forEach(btn => {
    btn.addEventListener('click', (e) => {
      e.stopPropagation();
      const item = btn.closest('.act-item');
      if (!item) return;
      const agName = item.dataset.ag;
      // Find the decision tree details from AG_DATA (populated later in same closure)
      const entry = AG_DATA[agName];
      const details = entry?.approvals?.[0]?.details;
      if (!details) return;

      // Lazily create the inline expand container on first click
      ...
      if (!expand) {
        ...
        // Insert after the recommendation line, before the actions column
  ...
  function mkRows(scale) {
    ...
    const base = [
      ...
      { name: 'Main Brand Terms',   camp: 'Brand — Objection Experts', kw: 14, ads: 3, cost30: 47.60, impr30: 1960, clicks30: 136, cpc: 0.35, ctr: 6.94, conv30: 2, convRate: 1.47, flags: 1, flagType: 'concentration' },
  ...
  function flagBadgeHtml(type, agName) {
    if (!type) return '—';
    const labels = { negative: 'Negative Outlier', positive: 'Positive Outlier', pause: 'Pause Candidate', concentration: 'Concentration Alert' };
    return `<span class="flag-count-badge flag-count-badge--${type}" data-ag="${agName}" title="Click to view details"><span class="material-symbols-outlined">flag</span>${labels[type] || type}</span>`;
  }
