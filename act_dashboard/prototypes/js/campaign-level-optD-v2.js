/* ============================================================================
   ACT PROTOTYPE — CAMPAIGN LEVEL OPTION D v2 INTERACTIONS
   800px slide-in + full detail view. Proper review items in slide-in.
   ============================================================================ */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function openCampSlidein(name) {
    ...
    // Review items — using proper main-page components (Change 4)
    if (d.approvals.length > 0) {
      html += '<div class="camp-section-label">Awaiting Approval</div>';
      d.approvals.forEach(a => {
        html += `<div class="act-item" data-priority="investigate"><div class="act-item__row"><div class="act-item__content"><div class="act-item__top"><span class="badge-level badge-level--campaign">Campaign</span><span class="badge-action badge-action--investigate">Investigate</span><span class="badge-risk badge-risk--low">Low Risk</span></div><div class="act-item__summary">${a.text}</div><div class="act-item__recommendation"><span class="material-symbols-outlined">lightbulb</span>ACT recommends: ${a.type} optimization</div><div class="act-item__impact"><span class="material-symbols-outlined">trending_up</span>Estimated: ${a.impact}</div></div><div class="act-item__actions"><button class="btn-act btn-act--approve" onclick="this.closest('.act-item').style.opacity='0.4';this.closest('.act-item').style.pointerEvents='none';">Approve</button><button class="btn-act btn-act--decline" onclick="this.closest('.act-item').style.opacity='0.4';this.closest('.act-item').style.pointerEvents='none';">Decline</button></div></div></div>`;
      });
    }
    ...
    if (d.executed.length > 0) {
      ...
      d.executed.forEach(e => {
        html += `<div class="act-item"><div class="act-item__row"><div class="act-item__content"><div class="act-item__top"><span class="badge-level badge-level--campaign">Campaign</span><span class="badge-action badge-action--act">Act</span><span class="act-item__timestamp">${e.time}</span></div><div class="act-item__summary">${e.text}</div></div></div></div>`;
      });
    }

    // Universal Levers (Change 5: renamed from "Lever Status")
    html += '<div class="camp-section-label">Universal Levers</div><div class="lever-summary">';
    ...
    // Full detail button — closes slide-in and shows full detail view (Change 1)
    html += '<button class="btn-full-detail" onclick="closeCampSlidein(); showFullDetail()"><span class="material-symbols-outlined">open_in_new</span>View Full Detail</button>';
  ...
  // ── FULL DETAIL VIEW TOGGLE (Change 1) ──
  window.showFullDetail = function() {
    document.getElementById('listView').style.display = 'none';
    document.getElementById('fullDetailView').style.display = 'block';
    window.scrollTo(0, 0);
    // Re-bind collapse handlers for new DOM elements
    document.querySelectorAll('#fullDetailView .acct-section__header, #fullDetailView .act-section__header').forEach(header => {
      header.addEventListener('click', (e) => {
    ...
    document.querySelectorAll('#fullDetailView .lever-card__header').forEach(h => {
      h.addEventListener('click', () => h.closest('.lever-card').classList.toggle('collapsed'));
    });
    document.querySelectorAll('#fullDetailView .act-group__header').forEach(h => {
      h.addEventListener('click', (e) => { e.stopPropagation(); h.closest('.act-group').classList.toggle('collapsed'); });
    });
    document.querySelectorAll('#fullDetailView [data-action="approve"]').forEach(b => {
      b.addEventListener('click', (e) => { e.stopPropagation(); const i=b.closest('.act-item'); if(i){i.style.opacity='0.4';i.style.pointerEvents='none';} showToast('Approved','success'); });
    });
    document.querySelectorAll('#fullDetailView [data-action="decline"]').forEach(b => {
      b.addEventListener('click', (e) => { e.stopPropagation(); const i=b.closest('.act-item'); if(i){i.style.opacity='0.4';i.style.pointerEvents='none';} showToast('Declined','info'); });
    });
  };

  document.getElementById('backToList')?.addEventListener('click', () => {
    document.getElementById('fullDetailView').style.display = 'none';
