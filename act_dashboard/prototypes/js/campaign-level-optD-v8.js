  ...
  function openCampSlidein(name) {
    ...
    if (d.monitoring && d.monitoring.length > 0) {
      ...
      d.monitoring.forEach(m => {
        ...
        html += `<div class="act-item"><div class="act-item__row"><div class="act-item__content"><div class="act-item__top"><span class="badge-action badge-action--monitor">Monitor</span></div><div class="act-item__summary">${m.text}</div><div class="act-item__started"><span class="material-symbols-outlined">calendar_today</span>Started: ${m.started}</div><div class="act-item__progress"><div class="act-item__progress-bar"><div class="act-item__progress-fill" style="width:${m.progress}%;background:${barColor}"></div></div><span class="act-item__progress-text">${m.remaining}</span><span class="health-label ${labelClass}">${m.healthLabel}</span></div></div></div></div>`;
      });
      html += '</div></div>';
    }
    ...
    d.levers.forEach((l, i) => {
      ...
      if (l.guardrail) detailHtml += `<div class="lever-guardrail-note"><span class="material-symbols-outlined">shield</span>${l.guardrail}</div>`;
    ...
    html += '<div class="lever-guardrail-note" style="margin-top:12px"><span class="material-symbols-outlined">shield</span>Max 10% per cycle · 14-day cooldown · Loosening requires approval</div>';
    html += '</div></div>';
