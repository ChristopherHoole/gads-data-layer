/* ============================================================================
   ACT PROTOTYPE — CAMPAIGN LEVEL OPTION D v3 INTERACTIONS
   Slide-in IS the detail. No Campaign pills, impact in badge row, proper sections.
   ============================================================================ */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const CAMP_DATA = {
    'GLO Campaign — Core': { strat: 'tCPA £25', role: 'cp', status: 'enabled', budget: '£30/day', cost: '£487', cpa: '£18.50', cpaZone: 'outperforming', cpaDetail: '26% below target', conv: '26', convDetail: 'vs 22 last period (+18%)', qs: '6.2',
      levers: [
        { icon:'devices', name:'Device Modifiers', status:'Mobile +10%, Desktop +0%, Tablet -35%', cooldown:'Tablet: 5d remaining', type:'table', cols:['Device','Modifier','Last Changed','Cooldown'], rows:[['Mobile','+10%','1 Apr','Ready'],['Desktop','+0%','—','Ready'],['Tablet','-35%','5 Apr','5d remaining']] },
        { icon:'location_on', name:'Geographic', status:'5 locations, Scotland -50% (cap)', cooldown:'All ready', type:'table', cols:['Location','Modifier','Last Changed','Cooldown'], rows:[['England','+0%','—','Ready'],['Wales','-10%','3 Apr','26d remaining'],['Scotland','-50% (cap)','2 Apr','28d remaining'],['N. Ireland','-15%','3 Apr','26d remaining'],['London','+10%','1 Apr','Ready']] },
        { icon:'schedule', name:'Ad Schedule', status:'Sunday -20%, Saturday -10%', cooldown:'Sunday: 25d remaining', type:'table', cols:['Day/Time','Modifier','Last Changed','Cooldown'], rows:[['Sunday all day','-20%','5 Apr','25d remaining'],['Saturday all day','-10%','5 Apr','25d remaining'],['Mon 2-5pm','+15%','—','Ready']] },
      ...
      approvals: [
        { type: 'Bid Strategy', text: '<strong>Loosen tCPA</strong> £25 → £27.50 — CPA 26% below target for 14 days', rec: 'Increase target to capture more volume while CPA is outperforming', impact: '+2-3 conversions/week' },
        { type: 'Geographic', text: '<strong>Scotland geo</strong> +0% → -15% — CPA 45% above target over 30 days', rec: 'Apply -15% geo modifier for Scotland to reduce wasted spend', impact: '£4/week saving' },
        { type: 'Schedule', text: '<strong>Monday schedule</strong> +0% → +15% — Monday CPA £14.20 vs £22.50 avg', rec: 'Boost Monday bids to capture more volume on best-performing day', impact: '+1 conversion/week' },
      ],
      executed: [
        { text: '<strong>Sunday schedule</strong> +0% → -20%', time: '8 Apr, 05:14 AM', before: '+0%', after: '-20%' },
        { text: '<strong>Tablet device</strong> -20% → -35%', time: '8 Apr, 05:15 AM', before: '-20%', after: '-35%' },
  ...
  function openCampSlidein(name) {
    ...
    // ── AWAITING APPROVAL (Change 4: identical section header to main page) ──
    if (d.approvals.length > 0) {
      html += `<div style="display:flex;align-items:center;gap:8px;padding:12px 0 8px;border-bottom:1px solid var(--act-border-light);margin-bottom:0"><span class="material-symbols-outlined" style="font-size:20px;color:var(--act-amber)">pending_actions</span><span style="font-size:16px;font-weight:600;color:var(--act-text)">Awaiting Approval</span><span style="font-size:12px;font-weight:600;padding:2px 8px;border-radius:10px;color:white;background:var(--act-amber)">${d.approvals.length}</span></div>`;
      d.approvals.forEach(a => {
        // Change 2: no Campaign pill. Change 3: impact in badge row. Change 6: proper recommendation
        html += `<div class="act-item" data-priority="investigate"><div class="act-item__row"><div class="act-item__content"><div class="act-item__top"><span class="badge-action badge-action--investigate">Investigate</span><span class="badge-risk badge-risk--low">Low Risk</span><span class="act-item__impact"><span class="material-symbols-outlined">trending_up</span>${a.impact}</span></div><div class="act-item__summary">${a.text}</div><div class="act-item__recommendation"><span class="material-symbols-outlined">lightbulb</span>ACT recommends: ${a.rec || a.type + ' optimization'}</div></div><div class="act-item__actions"><button class="btn-act btn-act--approve" onclick="this.closest('.act-item').style.opacity='0.4';this.closest('.act-item').style.pointerEvents='none';">Approve</button><button class="btn-act btn-act--decline" onclick="this.closest('.act-item').style.opacity='0.4';this.closest('.act-item').style.pointerEvents='none';">Decline</button></div></div></div>`;
      });
    }

    // ── ACTIONS EXECUTED OVERNIGHT (Change 5: renamed, identical header) ──
    if (d.executed.length > 0) {
      html += `<div style="display:flex;align-items:center;gap:8px;padding:12px 0 8px;border-bottom:1px solid var(--act-border-light);margin-top:16px"><span class="material-symbols-outlined" style="font-size:20px;color:var(--act-green)">check_circle</span><span style="font-size:16px;font-weight:600;color:var(--act-text)">Actions Executed Overnight</span><span style="font-size:12px;font-weight:600;padding:2px 8px;border-radius:10px;color:white;background:var(--act-green)">${d.executed.length}</span></div>`;
      d.executed.forEach(e => {
        // Change 2: no Campaign pill
        html += `<div class="act-item"><div class="act-item__row"><div class="act-item__content"><div class="act-item__top"><span class="badge-action badge-action--act">Act</span><span class="act-item__timestamp">${e.time}</span></div><div class="act-item__summary">${e.text}</div>${e.before ? `<div class="act-item__meta"><span class="act-item__values"><span class="val-pill val-pill--old">${e.before}</span><span class="arrow">→</span><span class="val-pill val-pill--new-negative">${e.after}</span></span></div>` : ''}</div></div></div>`;
      });
    }

    // ── UNIVERSAL LEVERS (Change 5: renamed, Change 7: proper tables) ──
    html += `<div style="display:flex;align-items:center;gap:8px;padding:12px 0 8px;border-bottom:1px solid var(--act-border-light);margin-top:16px"><span class="material-symbols-outlined" style="font-size:20px;color:#10b981">tune</span><span style="font-size:16px;font-weight:600;color:var(--act-text)">Universal Levers</span></div>`;
    html += '<div class="lever-summary">';
    ...
    // ── STRATEGY SUMMARY ──
    html += `<div style="display:flex;align-items:center;gap:8px;padding:12px 0 8px;border-bottom:1px solid var(--act-border-light);margin-top:16px"><span class="material-symbols-outlined" style="font-size:20px;color:#10b981">target</span><span style="font-size:16px;font-weight:600;color:var(--act-text)">Bid Strategy</span></div>`;
