/* ============================================================================
   ACT PROTOTYPE — CAMPAIGN LEVEL OPTION D v4 INTERACTIONS
   Word-for-word match, collapsible sections, 6-col lever tables, impact on badge row.
   ============================================================================ */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const CAMP_DATA = {
    'GLO Campaign — Core': { strat: 'tCPA £25', role: 'cp', status: 'enabled', budget: '£30/day', cost: '£487', cpa: '£18.50', cpaZone: 'outperforming', cpaDetail: '26% below target', conv: '26', convDetail: 'vs 22 last period (+18%)', qs: '6.2',
      ...
      approvals: [
        { text: '<strong>Loosen tCPA target</strong> from £25 to £27.50 — CPA 26% below target for 14 days', rec: 'ACT recommends: Increase target to capture more volume', impact: 'Estimated: +2-3 conversions/week' },
        { text: '<strong>Reduce Scotland geo modifier</strong> from +0% to -15% — CPA 45% above target', rec: 'ACT recommends: Apply -15% geo modifier for Scotland', impact: 'Estimated saving: £4/week' },
        { text: '<strong>Increase Monday schedule bid</strong> from +0% to +15% — Monday CPA £14.20 vs £22.50 avg', rec: 'ACT recommends: Boost Monday bids for best-performing day', impact: 'Estimated: +1 conversion/week' },
      ],
      executed: [
  ...
  function openCampSlidein(name) {
    ...
    // ── Change 9: All sections collapsible. Approval EXPANDED, others COLLAPSED ──
    ...
    if (d.executed.length > 0) {
      ...
      d.executed.forEach(e => {
        html += `<div class="act-item"><div class="act-item__row"><div class="act-item__content"><div class="act-item__top"><span class="badge-action badge-action--act">Act</span><span class="act-item__timestamp">${e.time}</span></div><div class="act-item__summary">${e.text}</div>${e.before ? `<div class="act-item__meta"><span class="act-item__values"><span class="val-pill val-pill--old">${e.before}</span><span class="arrow">&rarr;</span><span class="val-pill val-pill--new-negative">${e.after}</span></span></div>` : ''}</div></div></div>`;
      });
      html += '</div></div>';
    }
    ...
    // Bind collapsible section headers (Change 9)
