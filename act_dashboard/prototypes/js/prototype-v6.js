/* ============================================================================
   ACT PROTOTYPE v6 — MORNING REVIEW INTERACTIONS
   v6: Empty states + all v3 behaviours
   ============================================================================ */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // -------------------------------------------------------------------------
  // -------------------------------------------------------------------------
  // v6: EMPTY STATE CHECKS
  // -------------------------------------------------------------------------
  function checkEmptyStates() {
    const sections = [
      { id: 'sectionApproval', icon: 'check_circle', text: 'All clear — no actions need your review today', extraClass: 'act-empty-state--approval' },
      { id: 'sectionExecuted', icon: 'bedtime', text: 'No actions were executed overnight.', extraClass: '' },
      { id: 'sectionMonitoring', icon: 'visibility_off', text: 'Nothing is currently being monitored.', extraClass: '' },
      { id: 'sectionAlerts', icon: 'notifications_none', text: 'No alerts in the last 7 days.', extraClass: '' },
    ];

    sections.forEach(({ id, icon, text, extraClass }) => {
      const section = document.getElementById(id);
      if (!section) return;
      const body = section.querySelector('.act-section__body');
      if (!body) return;
      const items = body.querySelectorAll('.act-item:not(.actioned)');
      const existing = body.querySelector('.act-empty-state');
      if (items.length === 0 && !existing) {
        // Hide groups and bulk bar
        body.querySelectorAll('.act-group').forEach(g => g.style.display = 'none');
        const bulkBar = section.querySelector('.act-bulk-bar');
        if (bulkBar) bulkBar.style.display = 'none';
        const empty = document.createElement('div');
        empty.className = `act-empty-state ${extraClass}`;
        empty.innerHTML = `<span class="material-symbols-outlined">${icon}</span><div class="act-empty-state__text">${text}</div>`;
        body.appendChild(empty);
      }
    });
  }

  // Run on load (for sections that start empty — currently none, but ready for dynamic use)
  checkEmptyStates();

  // Also re-check after approve/decline actions — override markActioned
  const origMarkActioned = markActioned;
  // We'll call checkEmptyStates after each approve/decline via a MutationObserver approach
  // But simpler: just call it after a short delay in the approve/decline handlers
  // Already handled by the fact that actioned items get .actioned class but stay in DOM
  // The empty state checks for :not(.actioned), so it works automatically
  // We just need to call checkEmptyStates after each action
  document.querySelectorAll('[data-action="approve"], [data-action="decline"]').forEach(btn => {
    btn.addEventListener('click', () => setTimeout(checkEmptyStates, 50), true);
  });
