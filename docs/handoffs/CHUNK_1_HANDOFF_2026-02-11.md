...
### 9.2) Step 2 — Refresh readonly DB (ingest first, then copy)
**Software: PowerShell**
1) Open PowerShell
2) Run the ingestion step (choose ONE mode):

**Option A — Test Account mode**
```powershell
...
3) (Optional) If Postgres metadata is required for this run, start it first:
```powershell
cd C:\Users\User\Desktop\gads-data-layer
docker compose up -d postgres
```

4) Refresh the readonly DuckDB (copy `warehouse.duckdb` → `warehouse_readonly.duckdb`):
```powershell
cd C:\Users\User\Desktop\gads-data-layer
