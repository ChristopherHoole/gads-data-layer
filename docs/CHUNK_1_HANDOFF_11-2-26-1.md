...
> **Contract rule:** In `analytics.*`, the canonical daily date column is always named **`snapshot_date`** (derived from Google Ads `segments.date`).
...
- Campaign daily performance (supports **mock + test modes**; production blocked until Basic Access approval)
...
## 9) Daily run workflow (Authoritative)

1) Close any apps that might lock the DB (e.g. DBeaver).

2) **Run ingestion** (choose one):

   * **Test account ingestion (real API test mode):**

     ```powershell
     python -m gads_pipeline.cli run-v1 test .\configs\client_001.yaml --lookback-days 1
     ```

   * **Mock ingestion (no credentials):**

     ```powershell
     python -m gads_pipeline.cli run-v1 mock .\configs\client_001.yaml --lookback-days 1
     ```

   Use **test** when you want a real Google Ads API pull (test account access). Use **mock** when you want a fully offline run with no credentials.

   **If ingestion fails, do not refresh readonly; fix ingestion first.**

3) Refresh the readonly DB:
...
5) Browse/query only `warehouse_readonly.duckdb` and only `analytics.*`.
...
- `customer_id`, `snapshot_date`, `campaign_id`, plus the entity keys for that dataset (e.g., `search_term` or `keyword_id`) and standard metrics (`clicks`, `impressions`, `cost_micros`, conversions fields).
...
SELECT
  snapshot_date,
  MAX(snapshot_date) OVER () AS max_snapshot_date,
  COUNT(*) AS rows
FROM analytics.campaign_daily
