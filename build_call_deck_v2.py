# -*- coding: utf-8 -*-
"""Call deck v2 — 4 slides, corrected stats, session-summary wins, 2 questions, Week 2 priorities."""
...
assert len(p.slides) == 4, f'Expected 4 slides, got {len(p.slides)}'
...
def find_by_geometry(slide, left_px, top_px, width_px_min=None, tol=40):
    """Find shape by pixel-approx geometry (1 px ≈ 9525 EMU)."""
    for sh in slide.shapes:
        l = (sh.left or 0) // 9525
        t = (sh.top or 0) // 9525
        w = (sh.width or 0) // 9525
        if abs(l - left_px) <= tol and abs(t - top_px) <= tol:
            if width_px_min is None or w >= width_px_min:
                return sh
    return None
...
def add_textbox(slide, left_px, top_px, width_px, height_px, lines, size=12, color=NAVY, bold=False):
...
# =============================================================
# SLIDE 1 — Week 1 Highlights (4 stat cards)
# =============================================================
s1 = p.slides[0]
...
# 4 stat cards — from v8 original: £8,462/87/101/£97 with labels
...
# =============================================================
# SLIDE 2 — What We Delivered (achievements from session summaries)
# =============================================================
s2 = p.slides[1]
...
# Add overlay text box for achievements (shape 169 is tf=False)
# Original container: (56, 148) 1162x249
add_textbox(s2, 58, 152, 1158, 260, [
...
# =============================================================
# SLIDE 3 — Questions for You (2 only)
# =============================================================
s3 = p.slides[2]
...
# Overlay question content on tf=False container (63, 148) 1164x283
add_textbox(s3, 65, 152, 1160, 300, [
    '1.  Bridges — what\'s the difference between a standard dental bridge and a Vivo bridge?',
], size=14, color=RED, bold=True)
add_textbox(s3, 65, 186, 1160, 60, [
...
# =============================================================
# SLIDE 4 — This Week's Priorities (from v8 Week 2 Plan)
# =============================================================
s4 = p.slides[3]
...
# Save clean (new file to eliminate zip duplicates)
...
for i, s in enumerate(p2.slides):
    ...
    print(f'  Slide {i+1}: {len(texts)} text shapes — title: "{texts[0][:60] if texts else "(empty)"}"')
