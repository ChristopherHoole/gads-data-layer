...
def setup_test_data(db_path: str = "warehouse.duckdb"):
    ...
    baseline = conn.execute(baseline_query, [
        customer_id, campaign_id, baseline_start, baseline_end
    ]).fetchone()
    ...
    print(f"  Budget: ${old_budget / 1_000_000:.2f} → ${new_budget / 1_000_000:.2f} (+10%)")
    ...
    existing = conn.execute(check_query, [customer_id, campaign_id, change_date]).fetchone()[0]
    ...
    change_id = conn.execute("SELECT MAX(change_id) FROM analytics.change_log").fetchone()[0]
    ...
    worsened_cpa = worsened_cost / worsened_conversions if worsened_conversions > 0 else 0
    worsened_roas = worsened_value / worsened_cost if worsened_cost > 0 else 0
    
    print(f"\n  Target worsened metrics:")
    print(f"    Cost: ${avg_cost / 1_000_000:.2f} → ${worsened_cost / 1_000_000:.2f} (+25%)")
    print(f"    Conversions: {avg_conversions:.1f} → {worsened_conversions:.1f} (-12%)")
    print(f"    CPA: ${baseline_cpa / 1_000_000:.2f} → ${worsened_cpa / 1_000_000:.2f} (+{((worsened_cpa/baseline_cpa - 1) * 100):.0f}%)")
    ...
    conn.execute(update_snap_query, [
        int(worsened_cost),
...
def test_monitoring(test_data: dict, config: dict):
    ...
    print(f"   Change: {test_change.old_value:.2f} → {test_change.new_value:.2f} ({test_change.change_pct:+.1%})")
    ...
    print(f"\n📊 Performance Delta:")
    print(f"   CPA: ${test_monitored.delta['baseline_cpa']:.2f} → ${test_monitored.delta['current_cpa']:.2f} ({test_monitored.delta['cpa_change_pct']:+.1%})")
    print(f"   Conversions: {test_monitored.delta['baseline_conversions']:.1f} → {test_monitored.delta['current_conversions']:.1f} ({test_monitored.delta['conversions_change_pct']:+.1%})")
    print(f"   ROAS: {test_monitored.delta['baseline_roas']:.2f} → {test_monitored.delta['current_roas']:.2f} ({test_monitored.delta['roas_change_pct']:+.1%})")
...
def test_triggers(monitored_change, config: dict):
    ...
    if not decision.should_rollback:
        ...
        print(f"   CPA change: {monitored_change.delta['cpa_change_pct']:+.1%} (need >+20%)")
        print(f"   Conv change: {monitored_change.delta['conversions_change_pct']:+.1%} (need <-10%)")
        return False
    ...
    if decision.trigger not in ['CPA_REGRESSION', 'ROAS_REGRESSION', 'VALUE_REGRESSION', 'ROAS_AND_VALUE_REGRESSION']:
...
def main():
    ...
    try:
        ...
        print("\nNext steps:")
        print("  1. Run real monitoring: python -m act_radar.cli check configs/client_synthetic.yaml")
        print("  2. Test rollback CLI: python -m act_radar.cli rollback configs/client_synthetic.yaml --dry-run")
        print("\n" + "=" * 80)
