...
from gads_pipeline.warehouse_duckdb import connect_warehouse, init_warehouse, insert_snap_campaign_daily
...
def main():
    parser = argparse.ArgumentParser(description="Generate synthetic campaign data for Lighthouse testing")
    parser.add_argument("--customer-id", required=True, help="Customer ID (e.g., 9999999999)")
    parser.add_argument("--days", type=int, default=60, help="Number of days to generate (default: 60)")
    parser.add_argument("--db", default="warehouse.duckdb", help="Database path (default: warehouse.duckdb)")
    ...
    print(f"  2. Run Lighthouse: python -m act_lighthouse.cli run-v0 configs/client_001.yaml --snapshot-date {end_date} --max-insights 8")
