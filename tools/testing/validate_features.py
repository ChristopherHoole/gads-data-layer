...
def fetch_raw_data(
    ...
    result = conn.execute(sql, [customer_id, campaign_id, start_date, snapshot_date]).fetchall()
...
def calculate_rolling_window(data: List[Dict], metric: str, window: int, target_date: date) -> Dict:
...
def calculate_derived_metric(data: List[Dict], window: int, target_date: date, metric_type: str) -> Optional[float]:
...
def validate_campaign_features(
    ...
    # Fetch raw data
    raw_data = fetch_raw_data(conn, customer_id, campaign_id, snapshot_date, lookback_days=60)
    ...
    # Get column names
    cols = [desc[0] for desc in conn.execute("SELECT * FROM analytics.campaign_features_daily LIMIT 0;").description]
    ...
    for metric in metrics:
        for w in windows:
            ...
            tolerance = max(0.01, abs(expected['sum']) * 0.0001) if expected['sum'] is not None else 0.01
            ...
            if sum_match and mean_match:
                ...
                status = "✗"
                exp_sum_str = f"{expected['sum']:.2f}" if expected['sum'] is not None else "None"
                act_sum_str = f"{actual_sum:.2f}" if actual_sum is not None else "None"
                exp_mean_str = f"{expected['mean']:.2f}" if expected['mean'] is not None else "None"
                act_mean_str = f"{actual_mean:.2f}" if actual_mean is not None else "None"
                print(f"{status} {metric}_w{w}_sum: expected={exp_sum_str}, actual={act_sum_str}")
                print(f"{status} {metric}_w{w}_mean: expected={exp_mean_str}, actual={act_mean_str}")
    
    print(f"✓ Rolling windows: {len(results['passed'])} passed, {len(results['failed'])} failed\n")
        ...
        for w in windows:
            ...
            if expected is None and actual_val is None:
                ...
                if abs(expected - actual_val) < 0.0001:
                    ...
                    results['failed'].append(f"{metric}_w{w}")
                    print(f"✗ {metric}_w{w}_mean: expected={expected:.6f}, actual={actual_val:.6f}, diff={abs(expected - actual_val):.6f}")
    
    print(f"✓ Derived metrics: {len(results['passed']) - len([x for x in results['passed'] if 'w' in x and any(m in x for m in metrics)])} passed\n")
    ...
    conv_30d = calculate_rolling_window(raw_data, 'conversions', 30, snapshot_date)['sum']
    ...
    expected_low_data_flag = expected_low_clicks or expected_low_conv or expected_low_impr
    ...
    if expected_low_clicks == actual_low_clicks:
        ...
        print(f"✗ low_data_clicks_7d: expected={expected_low_clicks}, actual={actual_low_clicks}")
        results['failed'].append('low_data_clicks_7d')
    
    if expected_low_conv == actual_low_conv:
        print(f"✓ low_data_conversions_30d: {actual_low_conv} (conv_30d={conv_30d:.0f})")
        results['passed'].append('low_data_conversions_30d')
    else:
        print(f"✗ low_data_conversions_30d: expected={expected_low_conv}, actual={actual_low_conv}")
    ...
    if expected_low_impr == actual_low_impr:
        ...
        print(f"✗ low_data_impressions_7d: expected={expected_low_impr}, actual={actual_low_impr}")
    ...
    if expected_low_data_flag == actual_low_data_flag:
        ...
        print(f"✗ low_data_flag: expected={expected_low_data_flag}, actual={actual_low_data_flag}")
...
def main():
    parser = argparse.ArgumentParser(description="Validate Lighthouse feature calculations")
    ...
    parser.add_argument("--db", default="warehouse_readonly.duckdb", help="Database path")
