"""
Generate synthetic ad_daily table - 90 DAYS VERSION.

Generates last 90 days only to stay under ~30k row limit.
This is sufficient for testing the ads.py route with period-over-period comparisons.
...
END_DATE = date(2026, 2, 21)  # End date (today in synthetic world)
START_DATE = END_DATE - timedelta(days=89)  # 90 days back
DAYS = 90

print("=" * 80)
print("SYNTHETIC AD_DAILY TABLE GENERATION - 90 DAYS")
print("=" * 80)
print(f"Customer ID: {CUSTOMER_ID}")
print(f"Date Range: {START_DATE} to {END_DATE} ({DAYS} days)")
print(f"Strategy: Generate last 90 days only (stays under row limit)")
print()
...
def generate_ad_performance(status, base_impressions, day_offset, total_days):
    ...
    learning_factor = 1.0  # Assume ads already learned (90 days old minimum)
    fatigue_factor = 1.0
...
print(f"Estimated total rows: {len(ads) * DAYS:,} (should be under 30,000)")
...
# Generate all data at once (small enough to fit in memory)
print(f"\n[4/5] Generating {DAYS} days of performance data...")
...
print(f"✓ Inserted {len(all_rows):,} rows into warehouse.duckdb")
...
print(f"Date range: {START_DATE} to {END_DATE} ({DAYS} days)")
print()
print("NOTE: Using 90 days instead of 365 to stay under row limit.")
print("This is sufficient for testing period-over-period metrics (30d vs prior 30d).")
print()
